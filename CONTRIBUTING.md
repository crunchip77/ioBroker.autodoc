...
- **W5005 (`setTimeout` in lib files):** Three sources are flagged — two are **intentional** and one is **fixed**:
  - **`lib/aiEnhancer.js`** (`postJsonTransientRetries` retry delay): **fixed** — `invokeProvider` in the `AiEnhancer` class passes `(ms) => new Promise(r => this.adapter.setTimeout(r, ms))` so the delay is registered with the adapter lifecycle (compact mode safe).
  - **`lib/htmlRenderer.js`** (lines 1099, 1347, 1616, 3997): **intentional false positive** — all four occurrences are inside JavaScript `<script>` template literal strings that are written into the generated HTML output. They run in the **browser**, not in the Node.js adapter process. The repochecker does a text search and cannot distinguish browser-side JS embedded in template strings from adapter runtime code. Do **not** replace these with `this.setTimeout` — that would break the browser UI.
  - **`lib/htmlToPdf.js`** (`delay()` sleep): **intentional** — a one-shot ≤ 500 ms sleep during PDF rendering after all HTML is loaded. No recurring timer; if the adapter stops, the short promise completes or is abandoned without holding the process. Not suitable for `this.setTimeout` as this is a standalone utility function without adapter context.

...
| **npm-Release** + **ioBroker.repositories** | ✅ | **npm** **`iobroker.autodoc`** **0.9.38**; **latest**-Eintrag **gemerged** — [TODO § 1.1](TODO.md#release-veroeffentlichung), [CONTRIBUTING — npm](CONTRIBUTING.md) |
...
**Umsetzung (Stand Code):** Kanonische Dateien bleiben unter **`/files/`**. Konfiguration **`documentationStatesMode`**: **`full`** (Volltext in States) oder **`metadata`** (Platzhalter in großen `documentation.*`-States). **Neue Instanzen (ab io-package 0.9.38):** Default **`metadata`**; bestehende behalten den gespeicherten Wert. Download-Aktionen lesen aus Dateien (`autodoc-latest.*` …), mit Fallback für alte Installationen. Zusätzlich State **`documentation.exportHashes`** (SHA-256 hex der drei „latest“-Exporte) für Änderungserkennung **in beiden Modi**. Details und Checkboxen: [TODO.md — Anhang A](TODO.md#anhang-a-erledigt).
...
| **States / Redis** | Kanonisch weiterhin Dateien unter `/files/`; **Default `documentationStatesMode` = `metadata`** für **neue** Instanzen (**0.9.38**, mit `common.news`). **`full`** bleibt wählbar. Keine **großen Binärdateien** (Fotos, große PNG) bewusst über AutoDoc in die virtuelle Dateischicht legen — **besonders bei Redis**; jsonl ist toleranter, **einheitliche Nutzer-Empfehlung** bleibt dieselbe. |
...
| **npm/stable** | **npm** + **ioBroker.repositories** (**latest**) für **autodoc** ✅ (**0.9.38**, neue Instanzen **States**-Default **`metadata`**)                                         | ✅ erledigt | Folge-Versionen: npm + Listen-Update nach **CONTRIBUTING** / repositories-Doku |

...
const translations = {
	en: {
		...
		documentationScore: 'Documentation setup score',
		instancesWithoutRoom: 'Instances not assigned to any room',
		checklistProjectNarrative:
			'“My documentation” project description is shorter than the minimum configured under Adapter → Advanced (documentation setup score). Clear sentences help readers and exports.',
		checklistBaseUrlUnset:
			'ioBroker base URL is empty under Adapter → Advanced — QR codes and documentation bookmark links may not work from other devices or networks.',
		scriptsWithoutDescription: 'Scripts without common.desc (group-purpose)',
		scriptsWithoutDescOptionalCaption:
			'Informational: these active scripts have no common.desc on the script object — in ioBroker that field is optional group-purpose text (typically global scripts), not a household “description”. Never part of the documentation setup score.',
		...
		allGood: 'No enabled checklist rows need attention right now.',
		checkOk: 'OK',
		...
		diagFindingScripts: n =>
			`${n} script(s) have no common.desc on the script object (optional group-purpose field — useful for readers/AI, not required)`,
		...
		mermaidDiagramIntro:
			'Diagram from your documentation settings (Mermaid). Exported HTML embeds SVG when the optional @mermaid-js/mermaid-cli package is installed; otherwise the browser loads Mermaid from jsDelivr. Markdown keeps the source for Mermaid-capable viewers.',
		mermaidAutoTopologyTitle: 'Host topology (auto-generated)',
		mermaidAutoTopologyIntro:
			'Adapter instances grouped by ioBroker host, with a fixed maximum number of nodes for readability. This is not a full wiring or dependency graph. Hide with chapter id mermaid (same as your own Mermaid diagram).',
		...
		aliasTarget: 'Target',
		scoreDesc:
			'The percentage is the **documentation setup score**: share of **enabled** checklist rows below that pass (meta only). It does **not** judge script or room “content quality”. Disabled adapter instances are inventory only. The optional script field common.desc is never part of this score.',
		totalSuffix: 'total',
	...
	de: {
		...
		documentationScore: 'Doku-Setup-Score',
		instancesWithoutRoom: 'Instanzen ohne Raumzuweisung',
		checklistProjectNarrative:
			'Die Projektbeschreibung unter „Meine Dokumentation“ unterschreitet die unter Adapter → Erweitert eingestellte Mindestlänge (Doku-Setup-Score). Einige klare Sätze helfen Lesern und Exporten.',
		checklistBaseUrlUnset:
			'Unter Adapter → Erweitert ist keine ioBroker-Basis-URL gesetzt — QR-Code und Lesezeichen-Links zur Doku sind von anderen Geräten oder Netzen ggf. nicht nutzbar.',
		scriptsWithoutDescription: 'Skripte ohne common.desc (Gruppenzweck)',
		scriptsWithoutDescOptionalCaption:
			'Hinweis: diese aktiven Skripte haben kein common.desc am Skriptobjekt — in ioBroker ist das ein optionales Feld für den Gruppenzweck (typisch globale Skripte), keine „Haushalts-Beschreibung“. Nie Teil des Doku-Setup-Scores.',
		...
		allGood: 'Keine aktive Checklisten-Zeile erfordert gerade Aufmerksamkeit.',
		checkOk: 'OK',
		...
		diagFindingScripts: n =>
			`${n} Skript(e) ohne common.desc am Skriptobjekt (optionaler Gruppenzweck — für Leser/KI hilfreich, nicht Pflicht)`,
		...
		mermaidDiagramIntro:
			'Diagramm aus den Dokumentations-Einstellungen (Mermaid). Der HTML-Export bettet SVG ein, wenn das optionale Paket @mermaid-js/mermaid-cli installiert ist; sonst lädt der Browser Mermaid von jsDelivr. Im Markdown bleibt der Quelltext für Mermaid-fähige Viewer.',
		mermaidAutoTopologyTitle: 'Host-Topologie (automatisch)',
		mermaidAutoTopologyIntro:
			'Adapter-Instanzen nach ioBroker-Host gruppiert, mit einer festen Maximalknotenzahl zur Lesbarkeit. Kein vollständiger Verbindungs- oder Abhängigkeitsgraph. Ausblenden mit Kapitel-Id mermaid (wie das eigene Mermaid-Diagramm).',
		troubleshootPublicLinksIntro:
		...
		aliasTarget: 'Ziel',
		scoreDesc:
			'Der Prozentsatz ist der **Doku-Setup-Score**: Anteil der **eingeschalteten** Check-Zeilen unten, die bestehen (nur Meta). **Nicht** die inhaltliche Qualität von Skripten oder Räumen. Deaktivierte Instanzen sind nur Inventar. common.desc fließt nie in diesen Score ein.',
		totalSuffix: 'gesamt',
	...
	fr: {
		...
		documentationScore: 'Score de mise en place documentaire',
		instancesWithoutRoom: 'Instances sans pièce assignée',
		checklistProjectNarrative:
			'La description du projet (« Ma documentation ») est plus courte que le minimum réglé sous Adaptateur → Avancé (score de mise en place). Quelques phrases claires aident la lecture et les exports.',
		checklistBaseUrlUnset:
			'L’URL de base ioBroker est vide sous Adaptateur → Avancé — le QR code et les liens signets peuvent ne pas fonctionner depuis d’autres appareils ou réseaux.',
		scriptsWithoutDescription: 'Scripts sans common.desc (objectif de groupe)',
		scriptsWithoutDescOptionalCaption:
			'À titre informatif : ces scripts actifs n’ont pas de common.desc sur l’objet script — dans ioBroker ce champ optionnel sert à l’objectif de groupe (souvent scripts globaux), pas à une « description foyer ». Jamais dans le score de mise en place documentaire.',
		...
		allGood: 'Aucune ligne de checklist activée ne nécessite d’action pour l’instant.',
		checkOk: 'OK',
		...
		diagFindingScripts: n =>
			`${n} script(s) sans common.desc sur l’objet script (champ optionnel d’objectif de groupe — utile lecture/IA, pas obligatoire)`,
		...
		mermaidDiagramIntro:
			'Diagramme issu des paramètres de documentation (Mermaid). L’export HTML intègre du SVG si le paquet optionnel @mermaid-js/mermaid-cli est installé ; sinon le navigateur charge Mermaid via jsDelivr. Le Markdown conserve la source pour les lecteurs compatibles Mermaid.',
		mermaidAutoTopologyTitle: 'Topologie des hôtes (automatique)',
		mermaidAutoTopologyIntro:
			'Instances d’adaptateurs regroupées par hôte ioBroker, avec un nombre maximal de nœuds pour la lisibilité. Ce n’est pas un schéma complet des liaisons ou dépendances. Masquez avec l’id de chapitre mermaid (comme pour votre schéma Mermaid personnel).',
		troubleshootPublicLinksIntro:
		...
		aliasTarget: 'Cible',
		scoreDesc:
			'Le pourcentage est le **score de mise en place documentaire** : part des lignes **activées** ci‑dessous qui passent (méta uniquement). Ce n’est pas la « qualité » du contenu des scripts ou des pièces. Les instances désactivées sont inventaire seul. Le champ common.desc n’entre jamais dans ce score.',
		totalSuffix: 'total',

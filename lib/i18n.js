...
const translations = {
	en: {
		...
		tsDocNotGenerated2:
			'Note: Missing triggers explain the problem only if every automatic option is off (start off, adapter changes off, interval 0) — then enable at least one under Basic settings. If «generate on adapter start» and/or «on adapter changes» (or hours interval > 0) is already on, the cause is not forgotten checkboxes — check the adapter log, wait ~30 s after adapter events (debounce), or trigger manually (next item).',
	...
	de: {
		...
		tsDocNotGenerated2:
			'Fehlende Haken sind nur dann die Ursache, wenn alle automatischen Auslöser aus sind (Start aus, „bei Adapteränderungen“ aus, Intervall 0) — dann unter Grundeinstellungen mindestens einen aktivieren. Sind „beim Adapter-Start“ und/oder „bei Adapteränderungen“ (oder Intervall > 0) bereits aktiv, liegt es nicht an vergessenen Checkboxen — Adapter-Log prüfen, nach Adapteränderungen ca. 30 s warten (Debounce), oder unten manuell auslösen.',
	...
	fr: {
		...
		tsDocNotGenerated2:
			'À noter : des cases décochées n’expliquent l’absence de génération que si tous les déclencheurs automatiques sont désactivés (démarrage off, changements d’adaptateur off, intervalle 0) — alors activer au moins une option dans Paramètres de base. Si « au démarrage » et/ou « changements d’adaptateur » (ou intervalle > 0) est déjà activé, ce n’est pas une case oubliée — voir le log de l’adaptateur, attendre ~30 s après un événement adaptateur (debounce), ou le point suivant (déclenchement manuel).',

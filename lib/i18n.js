...
const translations = {
	en: {
		...
		tsDocNotGenerated2:
			'Under Basic settings, enable at least one automatic trigger — otherwise nothing starts by itself: generate on adapter start, generate on adapter changes (when adapters are installed/removed/enabled/disabled), and/or schedule interval hours > 0. Several options can be enabled at the same time.',
	...
	de: {
		...
		tsDocNotGenerated2:
			'Unter Grundeinstellungen muss mindestens ein automatischer Auslöser aktiv sein — sonst startet die Erzeugung nicht von selbst: „beim Adapter-Start“, „bei Adapteränderungen“ (Install/Entfernen/Aktivieren/Deaktivieren von Adaptern) und/oder „Intervall (Stunden)“ > 0. Mehrere Optionen gleichzeitig anzuhaken ist erlaubt und üblich.',
	...
	fr: {
		...
		tsDocNotGenerated2:
			'Dans Paramètres de base, au moins un déclencheur automatique doit être activé — sinon rien ne se lance tout seul : générer au démarrage, lors des changements d’adaptateur (installation/suppression/activation/désactivation), et/ou intervalle horaire > 0. Plusieurs options peuvent être cochées en même temps.',

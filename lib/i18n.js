...
const translations = {
	en: {
		...
		automationOverviewIntro:
			'Time-based automation only: JavaScript CRON (common.schedule), ioBroker schedule objects, and adapter CRON / restart schedules. Event-driven scripts (subscribe, Blockly, state changes) are listed in the Scripts chapter — not duplicated here.',
		automationOverviewEmpty:
			'No JavaScript CRON, schedule objects, or adapter CRON entries were found. Other scripts may still exist — see the Scripts chapter.',
		automationOverviewLimits:
			'Not included: event-driven JavaScript (see Scripts), Blockly logic details, Node-RED, scene adapters, and rules outside the sources above.',
		automationOverviewStatTotal: 'Scheduled entries',
		automationOverviewStatActive: 'Active',
		automationOverviewEventHint:
			'{0} additional active script(s) react to events (not on a clock) — see chapter Scripts for folders, filter, and inactive scripts.',
		automationTransparencyTitle: 'What AutoDoc cannot list as a schedule',
		automationTransparencyScriptsIntro: n =>
			n === 1
				? '1 active JavaScript automation — trigger mix:'
				: `${n} active JavaScript automations — trigger mix:`,
		automationTransparencyScriptsLink: 'Details in Scripts chapter',
		automationTransparencyRuleEnginesIntro:
			'These rule-engine adapters are installed but their flows/scenes are not parsed:',
		automationTransparencyFootnote:
			'Dynamic state IDs in script source, Blockly blocks, firmware rules, and vendor clouds may be missing. Only inventory from ioBroker objects and static script hints is shown.',
		automationTriggerBlockly: n => (n === 1 ? '1 Blockly' : `${n} Blockly`),
		automationTriggerSubscribe: n => (n === 1 ? '1 event-driven (subscribe)' : `${n} event-driven (subscribe)`),
		automationTriggerScheduleCode: n =>
			n === 1 ? '1 schedule() in code (no object CRON)' : `${n} schedule() in code (no object CRON)`,
		automationTriggerScheduleCron: n => (n === 1 ? '1 CRON on script object' : `${n} CRON on script object`),
		automationTriggerOnStart: n => (n === 1 ? '1 on start' : `${n} on start`),
		automationTriggerUnknown: n => (n === 1 ? '1 unknown trigger' : `${n} unknown trigger`),
		automationRuleEngineRow: (title, n) =>
			n === 1 ? `${title} — 1 enabled instance` : `${title} — ${n} enabled instances`,
		docTransparencyScriptStats: n =>
			n === 1
				? 'Active JavaScript automations (trigger mix):'
				: `${n} active JavaScript automations (trigger mix):`,
		automationOverviewSectionScriptCron: 'JavaScript — CRON (common.schedule)',
		automationOverviewSectionScheduleObject: 'Schedule objects (system schedule view)',
		automationOverviewSectionAdapterSchedule: 'Adapter instances — run mode schedule',
		automationOverviewSectionAdapterRestart: 'Adapter instances — restart CRON',
		automationOverviewDisabledGroup: 'Inactive entries ({0})',
		automationOverviewKind: 'Type',
		automationOverviewSource: 'Source',
		automationOverviewWhen: 'Schedule',
		automationOverviewStatus: 'Status',
		automationKindScriptCron: 'JavaScript (CRON)',
		automationKindScriptTrigger: 'JavaScript (trigger)',
		automationKindScheduleObject: 'Schedule object',
		automationKindAdapterSchedule: 'Adapter (run mode schedule)',
		automationKindAdapterRestart: 'Adapter (restart CRON)',
		active: 'active',
		inactive: 'inactive',
		...
		qsAutomationCount: n => (n === 1 ? '1 active automation (JavaScript)' : `${n} active automations (JavaScript)`),
		...
		qsRoomCardMore: n => `+ ${n} more in the rooms chapter`,
		...
		userAutomationsSummaryBody: n =>
			n === 1
				? '1 automation is active and runs in the background. Individual scripts without a description (common with Blockly) are not listed by file name here — to keep the family doc readable.'
				: `${n} automations are active and run in the background. Individual scripts without a description (common with Blockly) are not listed by file name here — to keep the family doc readable.`,
		userAutomationsOmittedNoDesc: n =>
			n === 1
				? '1 more active automation has no description in the documentation — see the summary below.'
				: `${n} more active automations have no description in the documentation — see the summary below.`,
		familyAutomationsTriggerMix: 'Type of active rules (no names):',
		...
		noDevicesInFunction: 'No devices assigned to this function.',
		roomDevices: 'devices',
		deviceSourceHint: name => `via ${name}`,
		...
		noAdaptersVisible: 'No connected systems to show.',
		...
		docTransparencyLimitsShort:
			'This documentation shows what AutoDoc can derive from ioBroker. Firmware behaviour, vendor clouds, Node-RED/scene/logic adapters (instances only — flows not parsed), Blockly internals, dynamic state IDs in script source, and automation outside captured sources may be incomplete or absent.',
	...
	de: {
		...
		automationOverviewTitle: 'Automatisierung im Überblick',
		automationOverviewIntro:
			'Nur zeitgesteuerte Automation: JavaScript-CRON (common.schedule), ioBroker-Schedule-Objekte sowie Adapter-CRON / Neustart-CRON. Ereignis-Skripte (subscribe, Blockly, State-Änderungen) stehen im Kapitel Skripte — hier nicht noch einmal als lange Liste.',
		automationOverviewEmpty:
			'Keine JavaScript-CRON-Einträge, Schedule-Objekte oder Adapter-CRON gefunden. Weitere Skripte können trotzdem existieren — siehe Kapitel Skripte.',
		automationOverviewLimits:
			'Nicht enthalten: ereignisgesteuerte Skripte (→ Skripte), Blockly-Details, Node-RED, Szenen-Adapter und Regeln außerhalb der Quellen oben.',
		automationOverviewStatTotal: 'Geplante Einträge',
		automationOverviewStatActive: 'davon aktiv',
		automationOverviewEventHint:
			'{0} weitere aktive Skript(e) reagieren auf Ereignisse (nicht nach Uhr) — Ordner, Filter und inaktive Skripte im Kapitel Skripte.',
		automationTransparencyTitle: 'Was AutoDoc nicht als Zeitplan auflisten kann',
		automationTransparencyScriptsIntro: n =>
			n === 1
				? '1 aktive JavaScript-Automatisierung — Auslöser-Mix:'
				: `${n} aktive JavaScript-Automatisierungen — Auslöser-Mix:`,
		automationTransparencyScriptsLink: 'Details im Kapitel Skripte',
		automationTransparencyRuleEnginesIntro:
			'Diese Regel-Engine-Adapter sind installiert; Abläufe/Szenen werden nicht ausgelesen:',
		automationTransparencyFootnote:
			'Dynamische State-IDs im Skript, Blockly-Blöcke, Firmware-Regeln und Hersteller-Clouds können fehlen. Es wird nur der ioBroker-Bestand und statische Skript-Hinweise gezeigt.',
		automationTriggerBlockly: n => (n === 1 ? '1 Blockly' : `${n} Blockly`),
		automationTriggerSubscribe: n =>
			n === 1 ? '1 ereignisgesteuert (subscribe)' : `${n} ereignisgesteuert (subscribe)`,
		automationTriggerScheduleCode: n =>
			n === 1 ? '1 schedule() im Code (ohne Objekt-CRON)' : `${n} schedule() im Code (ohne Objekt-CRON)`,
		automationTriggerScheduleCron: n => (n === 1 ? '1 CRON am Skript-Objekt' : `${n} CRON am Skript-Objekt`),
		automationTriggerOnStart: n => (n === 1 ? '1 beim Start' : `${n} beim Start`),
		automationTriggerUnknown: n => (n === 1 ? '1 unbekannter Auslöser' : `${n} unbekannte Auslöser`),
		automationRuleEngineRow: (title, n) =>
			n === 1 ? `${title} — 1 aktive Instanz` : `${title} — ${n} aktive Instanzen`,
		docTransparencyScriptStats: n =>
			n === 1
				? 'Aktive JavaScript-Automatisierungen (Auslöser-Mix):'
				: `${n} aktive JavaScript-Automatisierungen (Auslöser-Mix):`,
		automationOverviewSectionScriptCron: 'JavaScript — CRON (common.schedule)',
		automationOverviewSectionScheduleObject: 'Schedule-Objekte (system schedule)',
		automationOverviewSectionAdapterSchedule: 'Adapter-Instanzen — Laufmodus schedule',
		automationOverviewSectionAdapterRestart: 'Adapter-Instanzen — Neustart-CRON',
		automationOverviewDisabledGroup: 'Inaktive Einträge ({0})',
		automationOverviewKind: 'Art',
		automationOverviewSource: 'Quelle',
		automationOverviewWhen: 'Zeitplan',
		automationOverviewStatus: 'Status',
		automationKindScriptCron: 'JavaScript (CRON)',
		automationKindScriptTrigger: 'JavaScript (Auslöser)',
		automationKindScheduleObject: 'Schedule-Objekt',
		automationKindAdapterSchedule: 'Adapter (Laufmodus schedule)',
		automationKindAdapterRestart: 'Adapter (Neustart-CRON)',
		active: 'aktiv',
		inactive: 'inaktiv',
		...
		qsAutomationCount: n =>
			n === 1 ? '1 aktive Automatisierung (JavaScript)' : `${n} aktive Automatisierungen (JavaScript)`,
		...
		qsRoomCardMore: n => `+ ${n} weitere im Kapitel Räume`,
		...
		userAutomationsSummaryBody: n =>
			n === 1
				? '1 Automatisierung ist aktiv und läuft im Hintergrund. Einzelne Skripte ohne Beschreibungstext (häufig bei Blockly) werden hier nicht mit Dateinamen aufgelistet — damit die Familien-Doku verständlich bleibt.'
				: `${n} Automatisierungen sind aktiv und laufen im Hintergrund. Einzelne Skripte ohne Beschreibungstext (häufig bei Blockly) werden hier nicht mit Dateinamen aufgelistet — damit die Familien-Doku verständlich bleibt.`,
		userAutomationsOmittedNoDesc: n =>
			n === 1
				? '1 weitere aktive Automatisierung hat keine Beschreibung in der Doku — siehe Zusammenfassung unten.'
				: `${n} weitere aktive Automatisierungen haben keine Beschreibung in der Doku — siehe Zusammenfassung unten.`,
		familyAutomationsTriggerMix: 'Art der aktiven Regeln (ohne Namen):',
		...
		noDevicesInFunction: 'Dieser Funktion sind keine Geräte zugeordnet.',
		roomDevices: 'Geräte',
		deviceSourceHint: name => `über ${name}`,
		...
		noAdaptersVisible: 'Keine verbundenen Systeme sichtbar.',
		...
		docTransparencyLimitsShort:
			'Diese Dokumentation zeigt, was AutoDoc aus ioBroker ableiten kann. Verhalten in Geräte-Firmware, Hersteller-Clouds, Node-RED/Szenen/Logik-Adapter (nur Instanzen — Abläufe nicht ausgelesen), Blockly-Details, dynamische State-IDs im Skript und Automation außerhalb der erfassten Quellen kann unvollständig fehlen.',
	...
	fr: {
		...
		automationOverviewTitle: 'Vue d’ensemble des automatisations',
		automationOverviewIntro:
			'Automatisations horaires uniquement : CRON JavaScript (common.schedule), objets schedule ioBroker et CRON / redémarrage d’adaptateur. Les scripts événementiels (subscribe, Blockly) restent dans le chapitre Scripts — pas de liste dupliquée ici.',
		automationOverviewEmpty:
			'Aucun CRON JavaScript, objet schedule ou CRON d’adaptateur trouvé. D’autres scripts peuvent exister — voir le chapitre Scripts.',
		automationOverviewLimits:
			'Non inclus : scripts événementiels (→ Scripts), détails Blockly, Node-RED, adaptateurs scènes et règles hors des sources ci-dessus.',
		automationOverviewStatTotal: 'Entrées planifiées',
		automationOverviewStatActive: 'actives',
		automationOverviewEventHint:
			'{0} script(s) actif(s) supplémentaire(s) réagissent à des événements (pas à l’heure) — dossiers, filtre et scripts inactifs dans le chapitre Scripts.',
		automationTransparencyTitle: 'Ce qu’AutoDoc ne peut pas lister comme planning',
		automationTransparencyScriptsIntro: n =>
			n === 1
				? '1 automatisation JavaScript active — mix de déclencheurs :'
				: `${n} automatisations JavaScript actives — mix de déclencheurs :`,
		automationTransparencyScriptsLink: 'Détails dans le chapitre Scripts',
		automationTransparencyRuleEnginesIntro:
			'Ces adaptateurs moteur de règles sont installés ; flux/scènes non analysés :',
		automationTransparencyFootnote:
			'IDs d’état dynamiques dans le code, blocs Blockly, règles firmware et clouds fabricants peuvent manquer. Seuls l’inventaire ioBroker et les indices statiques des scripts sont affichés.',
		automationTriggerBlockly: n => (n === 1 ? '1 Blockly' : `${n} Blockly`),
		automationTriggerSubscribe: n => (n === 1 ? '1 événementiel (subscribe)' : `${n} événementiels (subscribe)`),
		automationTriggerScheduleCode: n =>
			n === 1 ? '1 schedule() dans le code (sans CRON objet)' : `${n} schedule() dans le code (sans CRON objet)`,
		automationTriggerScheduleCron: n => (n === 1 ? '1 CRON sur l’objet script' : `${n} CRON sur l’objet script`),
		automationTriggerOnStart: n => (n === 1 ? '1 au démarrage' : `${n} au démarrage`),
		automationTriggerUnknown: n => (n === 1 ? '1 déclencheur inconnu' : `${n} déclencheurs inconnus`),
		automationRuleEngineRow: (title, n) =>
			n === 1 ? `${title} — 1 instance activée` : `${title} — ${n} instances activées`,
		docTransparencyScriptStats: n =>
			n === 1
				? 'Automatisations JavaScript actives (mix de déclencheurs) :'
				: `${n} automatisations JavaScript actives (mix de déclencheurs) :`,
		automationOverviewSectionScriptCron: 'JavaScript — CRON (common.schedule)',
		automationOverviewSectionScheduleObject: 'Objets schedule (vue system schedule)',
		automationOverviewSectionAdapterSchedule: 'Instances adaptateur — mode schedule',
		automationOverviewSectionAdapterRestart: 'Instances adaptateur — CRON redémarrage',
		automationOverviewDisabledGroup: 'Entrées inactives ({0})',
		automationOverviewKind: 'Type',
		automationOverviewSource: 'Source',
		automationOverviewWhen: 'Planning',
		automationOverviewStatus: 'Statut',
		automationKindScriptCron: 'JavaScript (CRON)',
		automationKindScriptTrigger: 'JavaScript (déclencheur)',
		automationKindScheduleObject: 'Objet schedule',
		automationKindAdapterSchedule: 'Adaptateur (mode schedule)',
		automationKindAdapterRestart: 'Adaptateur (CRON redémarrage)',
		active: 'actif',
		inactive: 'inactif',
		...
		qsAutomationCount: n =>
			n === 1 ? '1 automatisation active (JavaScript)' : `${n} automatisations actives (JavaScript)`,
		...
		qsRoomCardMore: n => `+ ${n} de plus dans le chapitre Pièces`,
		...
		userAutomationsSummaryBody: n =>
			n === 1
				? '1 automatisation est active en arrière-plan. Les scripts sans texte de description (souvent Blockly) ne sont pas listés par nom de fichier ici — pour garder la doc famille lisible.'
				: `${n} automatisations sont actives en arrière-plan. Les scripts sans texte de description (souvent Blockly) ne sont pas listés par nom de fichier ici — pour garder la doc famille lisible.`,
		userAutomationsOmittedNoDesc: n =>
			n === 1
				? '1 autre automatisation active n’a pas de description dans la doc — voir le résumé ci-dessous.'
				: `${n} autres automatisations actives n’ont pas de description dans la doc — voir le résumé ci-dessous.`,
		familyAutomationsTriggerMix: 'Type de règles actives (sans noms) :',
		members: 'Membres',
		...
		noDevicesInFunction: 'Aucun appareil assigné à cette fonction.',
		roomDevices: 'appareils',
		deviceSourceHint: name => `via ${name}`,
		...
		noAdaptersVisible: 'Aucun système connecté à afficher.',
		...
		docTransparencyLimitsShort:
			'Ce document reflète ce qu’AutoDoc peut déduire d’ioBroker. Firmware, clouds fabricants, adaptateurs Node-RED/scènes/logique (instances seulement — flux non analysés), détails Blockly, IDs d’état dynamiques dans les scripts et automatisations hors sources capturées peuvent être incomplets ou absents.',

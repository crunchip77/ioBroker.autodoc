...
class MarkdownRenderer {
	...
	manualContextVisibleForMarkdown(docModel, profile) {
		...
		if (profile === PROFILE_USER) {
			...
			const mer =
				!this.userChapterHidden(docModel, 'mermaid') && (t(mc.mermaidDiagram) || t(mc.autoHostTopologyMermaid));
			const g =
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			const mer =
				!this.onboardingChapterHidden(docModel, 'mermaid') &&
				(t(mc.mermaidDiagram) || t(mc.autoHostTopologyMermaid));
			const g =
		...
		if (this.manualContextVisibleForMarkdown(docModel, profile)) {
			markdown += this.renderManualContext(
				docModel.manualContext,
				{
					...
					skipOwnerPlaybook:
						profile === PROFILE_USER
							? this.userChapterHidden(docModel, 'ownerPlaybook')
							: this.onboardingChapterHidden(docModel, 'ownerPlaybook'),
				},
				{
					adminTroubleshootLinks: false,
					docModelForSnapshot: docModel,
					troubleshootOmitProfile:
...
`;
	 ...
	 * @param {{ skipManualCore?: boolean, skipMermaid?: boolean, skipGuestHelp?: boolean, skipRoutines?: boolean, skipOwnerPlaybook?: boolean }} [skip]
...
`);
		...
		if (
			...
			const srcAt = String(manualContext.autoHostTopologyMermaid).trim();
...
`;
		...
		if (m.checklist.length === 0) {
			markdown += `_${i18n.t('maintenanceChecklistDisabled')}_\n\n`;
		}

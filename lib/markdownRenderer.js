...
const { buildDocChangeSinceLastRun, shouldShowUserFriendlyDocChange } = require('./docChangeFormat');
...
class MarkdownRenderer {
	...
	renderMarkdown(docModel) {
		...
		if (profile === PROFILE_USER) {
			markdown += this.renderUserSinceLastDocNoticeMarkdown(docModel);
	...
	buildAdminTocLineForKey(docModel, h, key, n) {
		...
		switch (key) {
			...
			case 'automationOverview': {
				if (h('automationOverview') || !docModel.automationOverview || docModel.automationOverview.isEmpty) {
					return null;
				}
				return `${n}. [${i18n.t('automationOverviewTitle')}](#automation-overview)`;
			}
			case 'scripts': {
				if (h('scripts')) {
					return null;
...
${
	docModel.scripts && docModel.scripts.enabledScripts > 0
		? `- **${i18n.t('enabledScripts')}:** ${docModel.scripts.enabledScripts}\n`
		: ''
}
### ${i18n.t('nextSteps')}
...
`;
		...
		if (
			...
			if (!block.skip) {
				markdown += `\n### ${i18n.t('docChangeSinceLastTitle')}\n\n`;
				markdown += `*${i18n.t('docChangeSinceLastNote')}*\n\n`;
				markdown += `${block.headline}\n\n`;
				if (block.lines.length > 0) {
					for (const row of block.lines) {
						markdown += `- **${row.typeLabel}:** ${row.detail}\n`;
					}
					markdown += '\n';
				}
			}
...
`;
		...
		if (profile === PROFILE_ADMIN) {
			markdown += `\n*${i18n.t('docTransparencyLimitsShort')}*\n`;
			...
			const breakdown =
				ao && ao.scriptStats && ao.scriptStats.enabledTotal
					...
					: '';
			if (breakdown) {
				markdown += `\n*${i18n.t('docTransparencyScriptStats', ao.scriptStats.enabledTotal)} ${breakdown}.*\n`;
			}
		}
		markdown += '\n---\n';
		return markdown;
	...
	/**
	 * Short User-profile Markdown notice when inventory deltas exist since last snapshot (non-technical).
	 *
	 * @param {object} docModel Document model
	 * @returns {string} Markdown fragment or empty
	 */
	renderUserSinceLastDocNoticeMarkdown(docModel) {
		if (!shouldShowUserFriendlyDocChange(docModel.docChangeSinceLastRun)) {
			return '';
		}
		const i18n = this.i18n;
		return `> ${i18n.t('userDocChangeSinceLastPlain')}\n\n`;
	}
...
`;
		...
		if (roomsData.totalRooms === 0) {
			...
			for (const room of roomsData.rooms) {
				...
				if (room.deviceCount != null && room.deviceCount !== room.memberCount) {
					markdown += `- **${i18n.t('roomDevices')}:** ${room.deviceCount}\n`;
				}
				...
				if (profile === PROFILE_ADMIN && room.devices.length > 0) {
					for (const member of room.devices) {
						...
						const sourceText =
							member.adapterName && member.deviceName
								? ` — ${i18n.t('deviceSourceHint', member.adapterName)}`
								: '';
						markdown += `  - ${member.deviceName}${sourceText} (\`${member.id}\`)${fnText}\n`;
					}
				}
				markdown += '\n';
			}
...
`;
	 ...
	 * @returns {string} Markdown fragment or empty
	 */
	_formatAutomationTransparencyMarkdown(overview) {
		const i18n = this.i18n;
		...
		let md = `### ${i18n.t('automationTransparencyTitle')}\n\n`;
		if (stats.enabledTotal > 0) {
			const breakdown = formatScriptTriggerBreakdown(stats, (key, ...args) => i18n.t(key, ...args));
			md += `${i18n.t('automationTransparencyScriptsIntro', stats.enabledTotal)}`;
			if (breakdown) {
				md += ` ${breakdown}.`;
			}
			md += ` → [${i18n.t('automationTransparencyScriptsLink')}](#scripts)\n\n`;
		}
		if (engines.length > 0) {
			md += `${i18n.t('automationTransparencyRuleEnginesIntro')}\n\n`;
			for (const e of engines) {
				md += `- ${i18n.t('automationRuleEngineRow', e.title, e.instanceCount)}\n`;
			}
			md += '\n';
		}
		md += `*${i18n.t('automationTransparencyFootnote')}*\n\n`;
		return md;
	}
	...
	renderAutomationOverviewChapter(docModel) {
		...
		const sectionDefs = [
			{ kind: 'script-cron', titleKey: 'automationOverviewSectionScriptCron' },
			{ kind: 'schedule-object', titleKey: 'automationOverviewSectionScheduleObject' },
			{ kind: 'adapter-schedule', titleKey: 'automationOverviewSectionAdapterSchedule' },
			{ kind: 'adapter-restart', titleKey: 'automationOverviewSectionAdapterRestart' },
		];

		let md = `<a id="automation-overview"></a>\n\n## ${i18n.t('automationOverviewTitle')}\n\n${i18n.t('automationOverviewIntro')}\n\n`;
		md += `**${i18n.t('automationOverviewStatTotal')}:** ${counts.total} · **${i18n.t('automationOverviewStatActive')}:** ${counts.enabled}`;
		if (scriptStats.enabledTotal > 0) {
			md += ` · **${i18n.t('enabledScripts')}:** ${scriptStats.enabledTotal}`;
		}
		md += '\n\n';
		md += this._formatAutomationTransparencyMarkdown(overview);
		...
		for (const def of sectionDefs) {
			...
			md += `### ${i18n.t(def.titleKey)} (${rows.length})\n\n`;
			md += `| ${i18n.t('automationOverviewSource')} | ${i18n.t('automationOverviewWhen')} | ${i18n.t('automationOverviewStatus')} |\n|---|---|---|\n`;
			for (const row of active) {
				const whenRaw = (row.detail || '—').replace(/\|/g, '\\|');
				md += `| **${row.name}** \`${row.id}\` | ${whenRaw} | ${i18n.t('active')} |\n`;
			}
			if (inactive.length > 0) {
				md += `\n*${i18n.t('automationOverviewDisabledGroup').replace('{0}', inactive.length)}:*\n\n`;
				for (const row of inactive) {
					const whenRaw = (row.detail || '—').replace(/\|/g, '\\|');
					md += `| **${row.name}** \`${row.id}\` | ${whenRaw} | ${i18n.t('inactive')} |\n`;
				}
			}
			md += '\n';
		}

		md += `*${i18n.t('automationOverviewLimits')}*\n\n---\n`;
		return md;
	}
...
`;
	 ...
	 * Omits interactive elements (forum copy button, JS); otherwise mirrors snapshot + narrow checks + maintenance reminder.
	 *
	 * @param {object} docModel Document model
	...
	renderDiagnosis(docModel) {
		...
		const nodeVer = system.primaryHost.nodeVersion;
...
${i18n.t('diagAutomatedChecksIntro')}

- ${nodeBullet}

### ${i18n.t('diagMaintenanceReminders')}

${i18n.t('diagMaintenanceRemindersIntro')}
...
${i18n.t('troubleshootingGenericDisclaimer')}

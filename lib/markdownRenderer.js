...
`;
	...
	renderAutomationOverviewChapter(docModel) {
		...
		md += `| ${i18n.t('automationOverviewKind')} | ${i18n.t('automationOverviewSource')} | ${i18n.t('automationOverviewWhen')} | ${i18n.t('automationOverviewStatus')} |\n|---|---|---|---|\n`;
		for (const row of overview.entries) {
			const whenRaw = (row.detail || row.triggerLabel || '—').replace(/\|/g, '\\|');
			const st = row.enabled ? i18n.t('active') : i18n.t('inactive');
			md += `| ${kindLabel(row.kind)} | **${row.name}** \`${row.id}\` | ${whenRaw} | ${st} |\n`;
		}
		md += `\n*${i18n.t('automationOverviewLimits')}*\n\n---\n`;
		return md;
	}

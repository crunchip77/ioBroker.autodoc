...
const { DEFAULT_ADMIN_CHAPTER_ORDER } = require('./docTemplateConfig');
...
class MarkdownRenderer {
	...
	manualContextVisibleForMarkdown(docModel, profile) {
		...
		if (profile === PROFILE_USER) {
			...
			const g = !this.userChapterHidden(docModel, 'guestHelp') && guestHelpChapterHasContent(mc, docModel);
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			const g = !this.onboardingChapterHidden(docModel, 'guestHelp') && guestHelpChapterHasContent(mc, docModel);
	...
	renderMarkdown(docModel) {
		...
		// User profile: same compact overview as User HTML (after TOC, before system)
		if (
			profile === PROFILE_USER &&
			!this.userChapterHidden(docModel, 'atAGlance') &&
			docModel.quickStart &&
			docModel.quickStart.hasContent
		) {
			markdown += this.renderUserAtAGlanceMarkdown(docModel);
		}
		...
		if (profile !== PROFILE_ONBOARDING) {
			const roomsHidden = profile === PROFILE_USER && this.userChapterHidden(docModel, 'rooms');
			if (!roomsHidden) {
		...
		if (profile !== PROFILE_ONBOARDING) {
			const scriptsHidden = profile === PROFILE_USER && this.userChapterHidden(docModel, 'scripts');
			if (!scriptsHidden) {
				markdown += this.renderScriptsChapter(docModel, profile);
			}
		}

		if (this.manualContextVisibleForMarkdown(docModel, profile)) {
			markdown += this.renderManualContext(
				...
				{ adminTroubleshootLinks: false, docModelForSnapshot: docModel },
			);
		}
		...
		if (profile !== PROFILE_ONBOARDING) {
			const troubleHidden = profile === PROFILE_USER && this.userChapterHidden(docModel, 'troubleshooting');
			if (!troubleHidden) {
	...
	renderAdminMarkdownKey(docModel, key) {
		switch (key) {
			case 'manual': {
				...
				return this.renderManualContext(docModel.manualContext, {}, { adminTroubleshootLinks: true });
			}
			case 'system': {
...
`;
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			const ob = [];
			if (!oh('quickstart')) {
				ob.push(`${n++}. [${i18n.t('quickStart')}](#quick-start)`);
			}
			if (!oh('system')) {
			...
			let n = 1;
			const ub = [];
			if (!uh('atAGlance') && docModel.quickStart && docModel.quickStart.hasContent) {
				ub.push(`${n++}. [${i18n.t('atAGlanceTitle')}](#at-a-glance)`);
			}
			if (!uh('system')) {
	...
	renderUserAtAGlanceMarkdown(docModel) {
		...
		const roomBits = (qs.roomGuides || [])
			.map(rg => {
				const head = `#### ${rg.name} (${i18n.t('qsRoomCardDevices', rg.deviceCount)})`;
				const hi = (rg.highlights || [])
					.map(h => {
						const val = h.valueText ? ` — ${h.valueText}` : '';
						return `- ${h.icon || '📦'} **${h.deviceName}**${val}`;
					})
					.join('\n');
				return `${head}\n\n${hi}`;
			})
...
### ${i18n.t('nextSteps')}
1. ${i18n.t('nextStepsReview')}
2. ${i18n.t('nextStepsManual')}
3. ${i18n.t('nextStepsAdapters')}
...
`;
	 ...
	 * @returns {string}
	 */
	renderTroubleshootGuestMarkdown(manualContext, includeAdmin) {
		const i18n = this.i18n;
		const t = v => v && String(v).trim();
		let out = '';
...
`;
	 ...
	 * @param {{ adminTroubleshootLinks?: boolean, docModelForSnapshot?: object }} [options] Admin Markdown: bookmark list; optional `docModelForSnapshot` for family diagnosis checklist in export
...
`);
		...
		if (!sk.skipGuestHelp) {
			const ts = this.renderTroubleshootGuestMarkdown(manualContext, includeAdminLinks);
			if (ts) {
				parts.push(ts);
			}
			if (snapModel) {

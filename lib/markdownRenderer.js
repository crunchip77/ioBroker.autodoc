		...
		if (this.manualContextVisibleForMarkdown(docModel, profile)) {
			markdown += this.renderManualContext(
				...
				},
				{
					...
					troubleshootOmitProfile: profile === PROFILE_USER ? 'user' : profile === PROFILE_ONBOARDING ? 'onboarding' : null,
				},
			);
		}
...
`;
	 ...
	 * @param {'user'|'onboarding'|null} [omitLinkForProfile] Same as HTML: no self-link to the current export profile
	 * @returns {string}
	 */
...
`;
	 ...
	 * @param {{ adminTroubleshootLinks?: boolean, docModelForSnapshot?: object, troubleshootOmitProfile?: 'user'|'onboarding'|null }} [options] Admin Markdown: bookmark list; optional `docModelForSnapshot` for family diagnosis checklist in export; `troubleshootOmitProfile` avoids self-link in User/Onboarding export

...
`;
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			if (!h('scripts')) {
				...
				const sharedStates = (scriptsData.stateCrossRef || []).filter(
					e => e.scripts && e.scripts.length > 1,
				);
				const sub = [];
				...
				lines.push(
					[`${n++}. [${i18n.t('scripts')}](#scripts)`, ...sub].filter(Boolean).join('\n'),
				);
			}
			...
			if (profile === PROFILE_ADMIN) {
				...
				if (scriptsWithRefs.length > 0) {
					...
					for (const script of scriptsWithRefs) {
						...
						const nameCell = mdTableCell(
							`${script.name} (${folderLbl})`,
						);

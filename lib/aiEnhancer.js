 ...
 * Skipped when documentation profile is admin. When enabled, runs two tailored calls
 * (user vs onboarding) so each HTML profile gets matching text — not a single shared block.
 */
...
Audience: guests and first-time visitors reading an onboarding page. Warm, inviting, easy to understand. No adapter IDs, OIDs, "instances", or admin jargon. Do not invent Wi‑Fi codes, alarm codes, or devices that are not supported by the data below.
...
Instructions:
1. NARRATIVE: Be generous — about 130–240 words (roughly 10–18 sentences, or 2–3 short paragraphs). Welcome them. Explain what this smart home offers in plain language using ONLY facts from the data: mention several rooms by name, hint what may run automatically (lights, climate, shutters, scenes — infer from functions/adapters/rooms, do not invent products). Help them feel oriented, not overwhelmed.
2. RECOMMENDATIONS: 7–10 bullet lines framed as friendly guest tips: orientation in the doc, that automations may run on schedules, who to ask for changes, comfort/safety common sense grounded in the setup. Still no invented secrets or devices.
...
class AiEnhancer {
	 ...
	 * Returns null if disabled, if documentation profile is admin, or if both calls fail.
	 *
	 * @param {object} docModel Document model
	...
	async enhance(docModel) {
		...
		const docProfile = (config.profile || 'admin').trim();
		if (docProfile === 'admin') {
			this.adapter.log.debug('AI enhancement skipped when documentation profile is admin');
			return null;
		}
		...
		const [userAi, onboardingAi] = await Promise.all([
			this.enhanceOneAudience(docModel, 'user', provider, model, langName),
			this.enhanceOneAudience(docModel, 'onboarding', provider, model, langName),
		]);

		if (!userAi && !onboardingAi) {
			return null;
		}

		return { user: userAi, onboarding: onboardingAi };
	}

...
function onboardingTextLooksLikeTechnicalDump(narrative, recommendations) {
	const b = `${narrative || ''}\n${recommendations || ''}`;
	if (/\bio\s*broker|\bioBroker\b/i.test(b)) {
		return true;
	}
	if (
...
const ONBOARDING_SYSTEM_MESSAGE_DE =
	'Du schreibst kurze Willkommenstexte für Gäste: natürliches Höflichkeits-Deutsch mit "Sie" in jedem Satz und in jeder Aufzählung — niemals "du", niemals du-Imperative wie "Lies", "Überprüfe", "Aktualisiere", "Halte", "Sei". Das Wort "ioBroker" und alles mit "Broker" als Produktname ist verboten (sage stattdessen "zu Hause", "die Steuerung hier", "dieses Zuhause"). Keine Adapter, Instanzen, Skript-Manager, maintenance-score, Wartungszahlen, BackItUp, Telegram-Adapter, Webinterface als Fachbegriff. Keine erfundenen Abläufe (Klima wärmt Essen, exakte Sonnenauf-/Untergangs-Szenen), wenn nicht klar aus den Fakten — lieber vorsichtig allgemein. Kein Denglish: kein Englisch mitten im Satz (kein "our", "home page(s)", keine halben englischen Sätze); kein "Schedule", "Score". Räume grammatikalisch richtig: **das** Arbeitszimmer, **das** Schlafzimmer — niemals "der Arbeitszimmer". Keine kaputten Wörter oder Pseudo-Komposita (z. B. "Schiebemögliche"); Fensterbeschattung: **Jalousien**, **Rollläden**, **Beschattung** — niemals "Blinde"/"Blinden" für Rollläden (falsche Bedeutung). Kein ganzer Text in markdown ** eingepackt; keine Meta-Schlussfloskel ("Ich hoffe, diese Vorschläge …"). Kein Software-Marketing ("freue mich Sie kennenzulernen", "komfortables Erlebnis") — Ton wie eine einladende Wohnung, nicht wie eine App. Keine Gästeansprache mit IT-Jargon (Algorithmen, sensibel im Tech-Sinn). Bewohner **bieten** Hilfe an / stehen für Fragen zur Verfügung — nicht formulieren, als würden die Bewohner "Hilfe anfragen". Keine sinnfremden Themen (Autobahn, Fernsehen, Nachrichten, Streaming) — nur Zuhause, Gäste, Bewohner, Komfort, Sicherheit. Schreibe nur über Räume und Gerätetypen, die im Prompt-Block "Grounding" vorkommen — keine Märchen, kein Verkauf, kein Gepäck, keine erfundenen Zimmernamen. Niemals "Vorgesetzte" oder "Ihre Vorgesetzten" im Privathaushalt — Ansprechpartner sind die Bewohner oder die hier Wohnenden.';

const USER_SYSTEM_MESSAGE_DE =
	'Du schreibst praxisnahe Smart-Home-Hinweise für Bewohner auf Deutsch: klar, natürlich, grammatikalisch korrekt — nicht hölzern übersetzt. Interne Host- oder Container-IDs (lange Hex-Zeichenketten) nicht wörtlich in den Fließtext übernehmen — sagen Sie z. B. „Ihr ioBroker“ oder nutzen Sie den Projektnamen. Technische Objektpfade weglassen; sinnvolle Alltagssprache. Keine Denglish-Wortformen (kein „Disablete“, keine erfundenen Komposita); keine doppelten Worthäufungen („Sicherheitssicherheit“). Ruhiger Ton bei Wartung — keine dramatischen Metaphern. Nicht pauschal fordern, „alle deaktivierten Adapter sofort einzuschalten“ — manche sind absichtlich aus; formulieren Sie vorsichtig („prüfen Sie, ob … beabsichtigt ist“). Markennamen nur wie in den Systemdaten (z. B. „Telegram-Bot“, nicht „telegramms“). Beschreibe nur Räume und Fähigkeiten aus dem "Grounding"-Block — keine erfundenen Sensoren oder Fantasie-Szenen. Keine nutzlosen Meta-Stichpunkte, die erklären, dass ioBroker "nur ein Programm" sei — Bewohner brauchen konkrete Pflegehinweise aus den Daten, keine Produktdefinitionen.';

/** Editor pass after onboarding generation (German only). */
const GERMAN_ONBOARDING_POLISH_SYSTEM =
	'Du bist deutscher Lektor für Gästetexte. Du änderst nur Anrede, Grammatik und holprige Formulierungen. Du erfindest keine neuen Fakten und keine neuen Produktnamen. Themenfremde oder unsinnige Stichpunkte (z. B. Autobahnverkehr, Fernsehen, Nachrichten) streichen oder durch einen sachlichen Gast-Hinweis zu diesem Zuhause ersetzen — nicht stehen lassen. Englisch in deutscher Ausgabe entfernen (z. B. "our home pages" → "in den Abschnitten auf dieser Seite" / "bei den Räumen unten"). "Der Arbeitszimmer" → "Das Arbeitszimmer"; kaputte Wörter wie "Schiebemögliche" durch sinnvolles Deutsch ersetzen. Falsche Übersetzung von Fenster-"blinds": "Blinde/Blinden" bei Rollläden → **Jalousien** oder **Rollläden**. Unsinn wie "Wetter einzuklagen" streichen oder durch sachliches Deutsch ersetzen. Meta-Schlussfloskeln ("Ich hoffe, diese Vorschläge …") und äußere markdown-**-Rahmen um ganze Absätze entfernen. **Vorgesetzte** / **von Ihren Vorgesetzten** in einem Privathaushalt durch **Bewohner** / **die hier wohnen** ersetzen — kein Büro-Jargon. Wenn Bewohner Hilfe anbieten: **Hilfe anbieten** / **für Fragen zur Verfügung stehen** — nicht "Hilfe anzufragen" für die Bewohner. Holpriges "Anstoß haben" → "ein Anliegen haben" o. Ä. "Wohnern" → "Bewohnern". Ausgabe immer exakt mit den Überschriften NARRATIVE: und RECOMMENDATIONS: wie vorgegeben.';
...
const GERMAN_USER_POLISH_SYSTEM =
	'Du bist deutscher Lektor für Bewohner-Hinweise zu einem Smart Home (ioBroker). Du korrigierst Grammatik und Anrede: durchgängig höfliches **Sie** mit richtigen Verbformen — keine **du**-Formen, keine Plural-Imperative an eine Person (**Überprüft …**, **Organisiert …**); stattdessen **Bitte prüfen Sie …**, **Überprüfen Sie …**. Entferne oder ersetze offensichtlich erfundene oder unsinnige Dinge (z. B. „Smart-Flasche“, „Wäschemöcke“, wirre Szenen), die nicht zu einem normalen Zuhause passen. Grammatik: **Ihr Smart Home** / **Ihr Zuhause**, nicht „Ihren Smart Home“; **auslesen**, nicht „auslese“ am Satzende. Keine wörtlichen Meta-Überschriften (**NARRATIVE**, **RECOMMENDATIONS**) im Fließtext — nur in der vorgegebenen Ausgabe-Struktur. Keine englischen Formatwörter im deutschen Text. Du erfindest keine neuen Adapter oder Marken. Ausgabe exakt mit NARRATIVE: und RECOMMENDATIONS: wie vorgegeben.';
...
function postJson(opts, body, timeoutMs = DEFAULT_REQUEST_TIMEOUT_MS) {
	return new Promise((resolve, reject) => {
		...
		req.setTimeout(timeoutMs, () => {
			req.destroy(new Error(`API request timed out after ${Math.round(timeoutMs / 1000)}s`));
		});
		req.write(payload);
		req.end();
	});
 ...
 * @returns {Promise<string>} Response text
 */
async function callOpenAiCompatible(baseUrl, apiKey, model, prompt, maxTokens, systemPrompt, temperature, timeoutMs) {
...
function buildSystemSummary(docModel) {
	...
	const scriptContextBlock =
		scriptContextLines.length > 0
			? `\nDocumented active scripts (script object common.desc — use only if relevant):\n${scriptContextLines.join('\n')}`
			: '';
	...
	const body = [
		...
		`Host: ${hostName} (${sys.primaryHost.platform}, ioBroker ${sys.primaryHost.version})`,
		...
		issueLines.length > 0 ? `Issues: ${issueLines.join('; ')}` : 'No disabled adapter instances in checklist.',
	].join('\n');

	return `${body}${scriptContextBlock}${hostHint}`;
}
...
class AiEnhancer {
	...
	async invokeProvider(config, provider, model, prompt, maxTokens, systemPrompt, temperature) {
		...
		if (provider === 'ollama') {
			...
			return callOpenAiCompatible(baseUrl, '', model, prompt, maxTokens, systemPrompt, temperature, timeoutMs);
		}
		if (provider === 'mistral') {
			const apiKey = (config.aiApiKey || '').trim();
			if (!apiKey) {
	...
	async enhanceOneAudience(docModel, audience, provider, model, langName, langCode) {
		...
		try {
			...
			if (
				...
				this.adapter.log.warn(
					'AI: onboarding text looked like admin/resident technical summary — replaced with neutral guest wording for autodoc-onboarding.html.',
				);

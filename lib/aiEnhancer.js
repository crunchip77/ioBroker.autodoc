 ...
 * @param {((ms: number) => Promise<void>) | undefined} [delayFn] Optional delay function; pass adapter.setTimeout-backed version for ioBroker lifecycle compliance
 * @returns {Promise<object>} Parsed JSON
 */
async function postJsonTransientRetries(opts, body, timeoutMs, log, delayFn) {
	const sleep = delayFn || (ms => new Promise(r => setTimeout(r, ms)));
	const maxAttempts = 4;
 ...
 * @param {((ms: number) => Promise<void>) | undefined} [delayFn] Delay function for retries (pass adapter.setTimeout-backed version)
 * @returns {Promise<string>} Response text
 */
...
class AiEnhancer {
	...
	async invokeProvider(config, provider, model, prompt, maxTokens, systemPrompt, temperature) {
		...
		// ioBroker-konform: adapter.setTimeout statt globalem setTimeout damit der Timer
		// beim Adapter-Stopp (compact mode) sauber beendet wird
		const delayFn = ms => new Promise(r => this.adapter.setTimeout(r, ms));

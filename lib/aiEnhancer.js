...
Instructions:
1. NARRATIVE: 5–8 sentences. Concrete overview tied to the real rooms/categories above and maintenance hints. Not generic filler. Stay matter-of-fact when mentioning maintenance or disabled components — no dramatic or surreal wording. Mention ioBroker only as plain household software the family uses, not as an admin audit report.
2. RECOMMENDATIONS: 5–8 bullet lines. Actionable habits tied to the data and issues above. If there are almost no issues, still give 3–4 positive, useful habits that fit the actual setup. Do **not** tell users to blindly enable every disabled adapter — some may be off on purpose; phrase as "check whether … is intentional". Only mention backup/messaging products if they appear in the system data; no invented "security audits".
...
function parseAiSections(text, langCode = 'en') {
	...
	if (/\*\*[^*\n]{2,80}\*\*/.test(t)) {
		...
		if (body.length > 25) {
			return { narrative: body, recommendations: fallbackRec };
		}
	}

	// Single block: use as narrative so export is not empty
	if (t.length > 25) {
		return { narrative: t.trim(), recommendations: fallbackRec };
	}

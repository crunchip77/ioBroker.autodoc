 ...
 */
async function callAnthropic(apiKey, model, prompt, maxTokens, systemPrompt, temperature) {
	const body = {
		model,
 ...
 * @returns {Promise<string>} Response text
 */
async function callOpenAiCompatible(baseUrl, apiKey, model, prompt, maxTokens, systemPrompt, temperature) {
...
class AiEnhancer {
	...
	async invokeProvider(config, provider, model, prompt, maxTokens, systemPrompt, temperature) {
		if (provider === 'anthropic') {
			...
			return callAnthropic(apiKey, model, prompt, maxTokens, systemPrompt, temperature);
		}
		if (provider === 'groq') {
			const apiKey = (config.aiApiKey || '').trim();
			if (!apiKey) {
		...
		if (provider === 'ollama') {
			...
			return callOpenAiCompatible(baseUrl, '', model, prompt, maxTokens, systemPrompt, temperature);
		}
		if (provider === 'mistral') {
			const apiKey = (config.aiApiKey || '').trim();
			if (!apiKey) {

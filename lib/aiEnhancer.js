 ...
 * Injected into both user and onboarding prompts as authoritative owner-provided facts.
 *
 * @param {object} docModel Document model
 * @returns {string} Formatted block or empty string
 */
function buildOwnerContextBlock(docModel) {
	...
	const hints = config.aiOwnerHints && config.aiOwnerHints.trim();
	if (hints) {
...
function buildAudiencePrompt(docModel, audience, langName, langCode) {
	...
	const ownerContext = buildOwnerContextBlock(docModel);

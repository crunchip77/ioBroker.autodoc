...
function buildSystemSummary(docModel) {
	...
	const body = [
		...
		issueLines.length > 0 ? `Issues: ${issueLines.join('; ')}` : 'Checklist: no warnings (disabled instances are inventory only).',
	].join('\n');
...
class AiEnhancer {
	...
	async enhanceScriptSources(docModel, rawData, provider, model, langName, _lang) {
		if (this.adapter.isScriptSourceAiCancelRequested()) {
			this.adapter.log.info('AI script source phase: cancel was already requested — skipping all script requests.');

...
function germanUserAiStillUnacceptable(narrative, recommendations) {
	...
	if (
		/iroBroker|IroBroker|\biro\s+broker\b|iRover|\bIr[oó]ver\b|Adapatoren?|Adapator|\bGerätee\b/i.test(b)
	) {
		return true;
	}
...
class AiEnhancer {
	...
	async enhanceOneAudience(docModel, audience, provider, model, langName, langCode) {
		...
		if (temperature === undefined && provider === 'ollama') {
			temperature =
				audience === 'onboarding'
					? OLLAMA_DEFAULT_TEMPERATURE_ONBOARDING
					: OLLAMA_DEFAULT_TEMPERATURE_USER;
		}
		try {

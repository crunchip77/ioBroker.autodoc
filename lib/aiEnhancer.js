 ...
 * Supported providers: anthropic, groq, ollama, google
...
Generate a short, friendly summary in ${langName} for the "${profile}" audience.
...
<text>`;
		...
		try {
			...
			if (provider === 'anthropic') {
				...
				const apiKey = (config.aiApiKey || '').trim();
				if (!apiKey) {
					this.adapter.log.warn('AI provider "google" selected but no API key configured');
					return null;
				}

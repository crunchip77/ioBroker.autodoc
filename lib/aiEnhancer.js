 ...
 * @param {number} [temperature] 0–1 (clamped); omit for API default
 * @param timeoutMs
 ...
 * @param {number} maxTokens Max response tokens
 * @param {string} [systemPrompt]
 * @param {number} [temperature] 0–2 (clamped); omit for API default
 * @param timeoutMs
 * @returns {Promise<string>} Response text
 */
...
GROUNDING (mandatory): Base every concrete claim on "System data" and the room capability block above. Only name rooms that appear there. Only describe home behaviours (lighting, heating, blinds, sensors, etc.) that match the listed categories for those rooms. Do not invent: garage pressure, scanning "bags", kitchen table occupancy, nonsense compounds (Lichtzuschnitt, Backup-Adaptation), fantasy products, furniture shops, backpacks, display cases, made-up room names, or surreal automation. If the export is sparse, write a shorter, honest summary instead of filler.

Instructions:
1. NARRATIVE: 5–8 sentences. Concrete overview tied to the real rooms/categories above and maintenance hints. Not generic filler. Stay matter-of-fact when mentioning maintenance or disabled components — no dramatic or surreal wording.
2. RECOMMENDATIONS: 5–8 bullet lines. Actionable habits tied to the data and issues above. If there are almost no issues, still give 3–4 positive, useful habits that fit the actual setup.
...
GROUNDING (mandatory for guests): Your text must reflect ONLY this home's named rooms, optional function themes, and the observed device categories in the grounding block. Welcome and tips may paraphrase in plain language — but do NOT invent objects, shopping, furniture, backpacks, refrigerators, overnight stays, mobile shops, fantasy room names, or domestic scenes that are not supported by those facts. If the home has few devices, write a SHORT calm welcome plus simple etiquette (ask hosts, sections below) instead of creative stories.

Instructions:
1. NARRATIVE: About 120–200 words max; shorter is fine if data is thin. Welcome them using only room names from the data. Mention capabilities only in broad household terms that match the categories (e.g. lighting, blinds, temperature) — never fake sensors or silly specifics.
...
class AiEnhancer {
		...
		try {
			...
			if (isAiBlockEmpty(parsed)) {
				...
				this.adapter.log.warn(
					`AI enhancement returned no usable text (${provider}, ${audience}) — model must output NARRATIVE: and RECOMMENDATIONS: sections. Raw excerpt: ${excerpt.replace(/\s+/g, ' ').trim()}`,
				);
				return null;
			}
			if (
	...
	async enhance(docModel) {
		...
		if (!o && u) {
			this.adapter.log.info(
				'AI: onboarding block missing — reusing user KI text for autodoc-onboarding.html (guest call failed or empty). Onboarding HTML will contain comment <!-- autodoc-ai:onboarding source=fallback-user -->.',
			);
	...
	parseResponse(text) {
		...
		// Split first: more reliable for long narratives than a single non-greedy match

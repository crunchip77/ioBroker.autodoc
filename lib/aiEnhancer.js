 ...
 * @param {(ms: number) => Promise<void>} delayFn Delay function backed by adapter.delay() for ioBroker lifecycle compliance
 ...
 * @param {(ms: number) => Promise<void>} delayFn Delay function for retries (pass adapter.delay()-backed version)
 * @returns {Promise<string>} Response text
 */
...
class AiEnhancer {
	...
	async invokeProvider(config, provider, model, prompt, maxTokens, systemPrompt, temperature) {
		...
		// ioBroker-konform: adapter.delay() aus der ioBroker-Adapter-Basisklasse —
		// Delay wird beim Adapter-Stopp (compact mode) sauber beendet
		const delayFn = ms => this.adapter.delay(ms);

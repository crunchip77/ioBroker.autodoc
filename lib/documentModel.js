...
class DocumentModel {
	...
	buildMaintenance(instances, rooms, scripts, manualContext, config, rawData) {
		...
		// At least one custom doc section has actual text content
		const customSections = parseCustomDocSections(c);
		const hasCustomSections = customSections.some(
			s => s.content && String(s.content).trim().length > 0,
		);
		...
		const aiProvider = String(c.aiProvider || '').trim().toLowerCase();

...
class DocumentModel {
	...
	async buildDocumentModel(rawData, trigger) {
		...
		const maintenance = this.buildMaintenance(filteredInstances);
	 ...
	 */
	buildMaintenance(instances) {
		// Disabled instances (inventory for the docs — not a quality issue; many installs disable adapters on purpose)
		const disabledInstances = instances.filter(inst => !inst.enabled);

		// Checklist: reserved for real warnings only (empty for now → score stays 100 %)
		const checklist = [];
	 ...
	 * guestHelpNote, homeRoutinesNote, adapterNotes table, roomNotes table) with fallback to legacy manualContext JSON.
	 *
	...
	parseManualContext(config) {
		...
		return { description, contact, notes, guestHelpNote, homeRoutinesNote, adapters, rooms };
	}

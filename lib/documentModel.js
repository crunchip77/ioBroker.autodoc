...
const { buildRoomDeviceList } = require('./roomDeviceResolver');
const {
...
const { buildAutomationOverview } = require('./automationOverview');
const { buildAutoHostTopologyMermaid } = require('./autoHostTopologyMermaid');
const { warnChapterJsonLayout } = require('./chapterConfigWarnings');
...
class DocumentModel {
	...
	async buildDocumentModel(rawData, trigger, options = {}) {
		...
		warnChapterJsonLayout(this.adapter.log, config);
	...
	buildRooms(rawData, deviceMap, liveStates) {
		...
		const roomList = rooms.map(room => {
			const devices = buildRoomDeviceList(room.members, deviceMap, liveStates, memberFunctions).map(dev => {
				const roleInfo = mapRole(dev.role || '');
				return {
					...dev,
		...
		const functionList = functions.map(fn => {
			const devices = buildRoomDeviceList(fn.members, deviceMap, liveStates, memberFunctions).map(dev => {
				const roleInfo = mapRole(dev.role || '');
				return {
					...dev,
					category: roleInfo.category,

...
/** At most this many script desc snippets in the system list — leaves room for room guide cards. */
const MAX_SCRIPT_ITEMS_IN_SYSTEM = 2;
/** Function areas beyond the top two rarely help guests; keeps Quick Start scannable. */
const MAX_FUNCTION_ITEMS_IN_SYSTEM = 2;
const MAX_ROOMS = 8;
...
/**
 * Within one room, prefer highlights that matter for orientation (guests / at-a-glance).
 * Lower = earlier when live-value tie-break is equal. Aligns loosely with {@link import('./roleMapper').mapRole} categories.
 */
const HIGHLIGHT_CATEGORY_RANK = {
	door: 0,
	window: 1,
	leak: 2,
	alarm: 3,
...
/**
 * @param {string | null | undefined} category Device `category` from discovery / role mapper
 * @returns {number} Sort rank for highlight ordering
 */
function highlightCategoryRank(category) {
	const c = category && String(category).trim() ? String(category).trim() : 'other';
	if (Object.prototype.hasOwnProperty.call(HIGHLIGHT_CATEGORY_RANK, c)) {
		return HIGHLIGHT_CATEGORY_RANK[c];
	}
	return HIGHLIGHT_CATEGORY_RANK.other;
}

/**
 * Sort key for room highlight candidates: live value first, then category rank, then device name.
 *
 * @param {{ currentValue?: unknown, category?: string, deviceName?: string } | null | undefined} a first device row
 * @param {{ currentValue?: unknown, category?: string, deviceName?: string } | null | undefined} b second device row
 * @returns {number} comparator result for Array.prototype.sort
 */
function compareQuickStartRoomDevices(a, b) {
	const as = a && a.currentValue != null && a.currentValue !== '' ? 1 : 0;
	const bs = b && b.currentValue != null && b.currentValue !== '' ? 1 : 0;
	if (bs !== as) {
		return bs - as;
	}
	const ra = highlightCategoryRank(a && a.category);
	const rb = highlightCategoryRank(b && b.category);
	if (ra !== rb) {
		return ra - rb;
	}
	const na = a && a.deviceName ? String(a.deviceName) : '';
	const nb = b && b.deviceName ? String(b.deviceName) : '';
...
function sliceQuickStartForOnboarding(qs) {
	...
	// Onboarding stat grid already shows enabled script count — skip duplicate automation line.
	const systemItems = (qs.systemItems || [])
		.filter(it => it && it.kind !== 'automationCount')
		.slice(0, ONBOARDING_VIEW_MAX_SYSTEM_ITEMS);
...
function buildQuickStartGuide(roomsBlock, scriptsBlock) {
	...
	fnList.sort((a, b) => {
		const dm = (b.memberCount || 0) - (a.memberCount || 0);
		if (dm !== 0) {
			return dm;
		}
	...
	const topFn = fnList.slice(0, MAX_FUNCTION_ITEMS_IN_SYSTEM);
	...
	const enabledScripts = scriptList.filter(s => s.enabled).length;
	if (enabledScripts > 0) {
		systemItems.push({
			kind: 'automationCount',
			n: enabledScripts,
		});
	}
	...
	for (const s of withDesc) {
		if (systemItems.length >= MAX_SYSTEM_ITEMS || scriptRows >= MAX_SCRIPT_ITEMS_IN_SYSTEM) {
			break;
		}
	...
	roomList.sort((a, b) => {
		const da = a && a.deviceCount != null ? a.deviceCount : a && Array.isArray(a.devices) ? a.devices.length : 0;
		const db = b && b.deviceCount != null ? b.deviceCount : b && Array.isArray(b.devices) ? b.devices.length : 0;
		if (db !== da) {
			return db - da;
		}
		const na = a && a.name ? String(a.name) : '';
	...
	for (const room of roomList) {
		...
		devs.sort(compareQuickStartRoomDevices);
		const byCat = new Map();
		const rest = [];
		for (const d of devs) {
			if (!d) {
			...
			const cat = (d.category && String(d.category).trim()) || 'other';
			if (!byCat.has(cat)) {
				byCat.set(cat, d);
			} else {
				rest.push(d);
		...
		picked.sort(compareQuickStartRoomDevices);
		const highlights = picked.slice(0, HIGHLIGHTS_PER_ROOM).map(d => ({
			deviceName: d.deviceName || '',
			icon: d.icon || '📦',
			category: (d.category && String(d.category).trim()) || '',
			valueText:
		...
		if (highlights.length > 0) {
			roomGuides.push({
				name: room.name,
				deviceCount: room.deviceCount != null ? room.deviceCount : devs.length,
				highlights,
			});
			roomIndex += 1;
		}
...
module.exports = {
	...
	compareQuickStartRoomDevices,
	HIGHLIGHT_CATEGORY_RANK,
	MAX_ROOMS,
	HIGHLIGHTS_PER_ROOM,
	MAX_SYSTEM_ITEMS,
	MAX_SCRIPT_ITEMS_IN_SYSTEM,
	MAX_FUNCTION_ITEMS_IN_SYSTEM,

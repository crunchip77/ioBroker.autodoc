...
`;
	...
	renderChangelogChapter(changelog) {
		...
		for (const entry of entries) {
			...
			html += `<div style="margin-bottom:16px;padding:12px;background:#f8f9fa;border-left:3px solid #0066cc;border-radius:0 4px 4px 0">
  ...
  <div style="font-size:13px;color:#555">${esc(entry.summary)}</div>`;
			if (entry.changes && entry.changes.length > 0) {
				html += `\n  <ul style="margin:6px 0 0 16px;font-size:12px;color:#666">\n`;
				for (const change of entry.changes.slice(0, 5)) {
					html += `    <li><span style="color:#0066cc;font-weight:600">${esc(change.type)}</span> ${esc(change.message)}</li>\n`;
				}
				if (entry.changes.length > 5) {
					html += `    <li style="color:#aaa">... ${entry.changes.length - 5} ${esc(i18n.t('moreChanges'))}</li>\n`;
				}
				html += `  </ul>`;
			}
			html += `\n</div>\n`;
		...
		if (changelog.length > 10) {
			html += `<p style="font-size:12px;color:#aaa">${esc(i18n.t('changelogTruncated').replace('{0}', changelog.length))}</p>\n`;
		}

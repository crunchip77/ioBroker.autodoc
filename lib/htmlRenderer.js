 ...
 */
const RENDERER_VERSION = '2026.09.20.1';
...
`;
  ...
  <span class="device-name">${esc(dev.deviceName)}</span>${liveHtml}
	...
	/**
	 * Admin: unified automation overview (scripts CRON/triggers, schedule objects, adapter CRON).
	 *
	 * @param {object} docModel Document model
	 * @returns {string} HTML
	 */
	renderAutomationOverviewChapter(docModel) {
		...
		const kindLabel = kind => {
			const map = {
				'script-cron': 'automationKindScriptCron',
				'script-trigger': 'automationKindScriptTrigger',
				'schedule-object': 'automationKindScheduleObject',
				'adapter-schedule': 'automationKindAdapterSchedule',
				'adapter-restart': 'automationKindAdapterRestart',
			};
			return i18n.t(map[kind] || kind);
		};
...
<div class="stat-grid">
  <div class="stat-card"><div class="num">${esc(counts.total)}</div><div class="label">${esc(i18n.t('automationOverviewTitle'))}</div></div>
  <div class="stat-card"><div class="num">${esc(counts.enabled)}</div><div class="label">${esc(i18n.t('enabledScripts'))}</div></div>
</div>
<table class="automation-overview-table">
<thead><tr>
  <th>${esc(i18n.t('automationOverviewKind'))}</th>
...
<tbody>
`;
		for (const row of overview.entries) {
			const whenRaw = row.detail || row.triggerLabel || '—';
			const whenDisplay =
				row.kind === 'script-cron' ||
				row.kind === 'adapter-schedule' ||
				row.kind === 'adapter-restart'
					? this.describeCron(whenRaw, i18n)
					: whenRaw;
			const whenTitle =
				whenRaw !== whenDisplay && String(whenRaw).trim()
			...
			html += `<tr>
  <td><small>${esc(kindLabel(row.kind))}</small></td>
  ...
  <td>${statusBadge}</td>
		...
		html += `</tbody>\n</table>
<p style="font-size:12px;color:var(--text-faint);margin-top:12px;border-left:3px solid var(--border);padding-left:10px">${esc(i18n.t('automationOverviewLimits'))}</p>
<hr class="section-divider">\n`;
		return html;
	}

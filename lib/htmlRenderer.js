 ...
 */
const RENDERER_VERSION = '2026.04.24.2';
...
${sub.join('\n')}
	...
	renderNav(profile, docModel) {
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			const scriptsNavItem = this.buildAdminScriptsNavItem(dm);
			if (scriptsNavItem) {
				lines.push(scriptsNavItem);
			}
...
`;
  ...
  <td><small>${esc(script.desc || '—')}</small>${aiExtra}</td>
			...
			if (profile === PROFILE_ADMIN) {
				...
				if (scriptsWithRefs.length > 0) {
					const refTotal = scriptsWithRefs.reduce((n, s) => n + (s.stateRefs && s.stateRefs.length ? s.stateRefs.length : 0), 0);

...
<h3>${esc(i18n.t('adapterDetails'))}</h3>
<table>
<thead><tr><th>${esc(i18n.t('name'))}</th>`;

		if (this.shouldShowDetail(profile, 'admin')) {
			html += `<th>${esc(i18n.t('totalInstances'))}</th><th>${esc(i18n.t('enabledInstances'))}</th>`;
		} else {
			html += `<th>Status</th>`;
		}
		...
		for (const adapter of adapters.adapters) {
			...
			html += `<tr><td><strong>${esc(adapter.name)}</strong>`;

			if (!config.hideInstanceDetailsInMarkdown && this.shouldShowDetail(profile, 'admin')) {
				// Show instance sub-rows as indented list inside cell
				const instanceItems = adapter.instances
					.map(inst => `<small>${esc(inst.id)} — <span class="badge ${inst.enabled ? 'badge-ok' : 'badge-off'}">${esc(inst.enabled ? i18n.t('enabled') : i18n.t('disabled'))}</span> ${esc(inst.version || '')}</small>`)
					.join('<br>');
				if (instanceItems) {
					html += `<br>${instanceItems}`;
				}
			}
			...
			if (this.shouldShowDetail(profile, 'admin')) {
				html += `<td>${esc(adapter.totalInstances)}</td><td>${esc(adapter.enabledInstances)}</td>`;
			} else {
				const active = adapter.enabledInstances > 0;
				html += `<td><span class="badge ${active ? 'badge-ok' : 'badge-off'}">${active ? 'Active' : 'Inactive'}</span></td>`;
			}

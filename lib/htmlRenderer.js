 ...
 */
const RENDERER_VERSION = '2026.04.07.05';
...
class HtmlRenderer {
	...
	renderOnboardingHtml(docModel) {
		...
		// 2. Tips & Notes — first content guests see after the intro
...
${descHtml}${contactHtml}<div style="clear:both"></div>
<hr class="section-divider">
`;
	}
...
`;
  ...
  <span class="device-name">${esc(dev.deviceName)}</span>${liveHtml}
		...
		// Functions overview — shown AFTER rooms as a capability summary
		if (roomsData.functions && roomsData.functions.length > 0) {
			const fnIcons = { 'licht': '💡', 'light': '💡', 'beleuchtung': '💡', 'rollladen': '🪟', 'shutter': '🪟', 'blind': '🪟', 'heizung': '🌡️', 'heating': '🌡️', 'thermostat': '🌡️', 'alarm': '🔔', 'sicherheit': '🔒', 'security': '🔒', 'steckdose': '🔌', 'socket': '🔌', 'kamera': '📷', 'camera': '📷', 'musik': '🎵', 'media': '🎵', 'garten': '🌿', 'garden': '🌿', 'klima': '❄️', 'climate': '❄️', 'ac': '❄️' };
			html += `<div style="margin-top:20px;padding:14px 16px;background:var(--surface);border-radius:8px;border:1px solid var(--border)">
<p style="font-size:13px;font-weight:600;color:var(--text-muted);margin-bottom:10px">⚙️ ${esc(i18n.t('availableFunctions') || 'Steuerbare Bereiche im ganzen Haus:')}</p>
<div style="display:flex;flex-wrap:wrap;gap:8px">\n`;
			for (const fn of roomsData.functions) {
				const key = fn.name.toLowerCase();
				const iconKey = Object.keys(fnIcons).find(k => key.includes(k));
				const icon = iconKey ? fnIcons[iconKey] : '⚙️';
				html += `<div style="background:var(--bg);border:1px solid var(--border);border-radius:20px;padding:5px 12px;font-size:13px;display:flex;align-items:center;gap:5px">
...
${system.hosts
	.map(h => {
		...
		// Prefer system RAM (native); fall back to process RAM (Docker-friendly)
		let ramHtml;
		if (res.sysTotalMb && res.sysFreeMb !== null) {
			const usedMb = res.sysTotalMb - res.sysFreeMb;
			ramHtml = `${usedMb} / ${res.sysTotalMb} MB`;
		} else if (res.procMb) {
			ramHtml = `~${res.procMb} MB <small style="color:var(--text-faint)">(${esc(i18n.t('processOnly') || 'Prozess')})</small>`;
		} else {
			ramHtml = '—';
		}
...
<summary class="maint-details-summary">${esc(room.name)} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${devCount} ${esc(i18n.t('members'))})</span></summary>

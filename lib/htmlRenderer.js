...
class HtmlRenderer {
	...
	renderUserHtml(docModel) {
		...
		const hasUserManualCore =
			uMc && !this.userChapterHidden(docModel, 'manual') && (uMc.description || uMc.contact || uMc.notes);
		const hasUserMermaid =
			uMc &&
			!this.userChapterHidden(docModel, 'mermaid') &&
			((uMc.mermaidDiagram && String(uMc.mermaidDiagram).trim()) ||
				(uMc.autoHostTopologyMermaid && String(uMc.autoHostTopologyMermaid).trim()));
		if (hasUserManualCore || hasUserMermaid) {
			body += this.renderManualContext(uMc, PROFILE_USER, docModel);
		}
		...
		if (!this.userChapterHidden(docModel, 'ownerPlaybook')) {
			body += this._renderOwnerPlaybookChapter(docModel.manualContext);
		}
	...
	renderOnboardingHtml(docModel, renderOptions) {
		...
		if (!this.onboardingChapterHidden(docModel, 'ownerPlaybook')) {
			body += this._renderOwnerPlaybookChapter(docModel.manualContext);
		}

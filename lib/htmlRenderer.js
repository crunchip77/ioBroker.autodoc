			...
			const buildAdapterRow = adapter => {
				...
				if (!config.hideInstanceDetailsInMarkdown) {
					const instRows = adapter.instances
						...
						.join('');
					if (instRows) {
						instanceDetailsHtml = `<details class="aci-instances"><summary>${esc(i18n.t('instanceDetails') || 'Instanzdetails')}</summary><div class="aci-instances-content">${instRows}</div></details>`;
					}
				}
				...
				const countsHtml = `<span class="aci-counts">${esc(adapter.totalInstances)}\u00a0/\u00a0${esc(adapter.enabledInstances)}\u00a0${esc(i18n.t('enabledShort') || 'aktiv')}</span>`;

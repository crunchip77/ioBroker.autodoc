...
class HtmlRenderer {
		...
		switch (key) {
			...
			case 'mermaid': {
				...
				const hasMermaid =
					uMc &&
					!h('mermaid') &&
					((uMc.mermaidDiagram && String(uMc.mermaidDiagram).trim()) ||
						(uMc.autoHostTopologyMermaid && String(uMc.autoHostTopologyMermaid).trim()));
				if (hasCore || hasMermaid) {
	 ...
	 * @param {{urls?:object, qrSvgs?:object}} [renderOptions]
	 * @returns {string} HTML fragment
	 */
...
${sub.join('\n')}
	...
	renderNav(profile, docModel) {
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			if (!oh('mermaid') && mcOb && mcOb.autoHostTopologyMermaid && String(mcOb.autoHostTopologyMermaid).trim()) {
				obLines.push(`<li><a href="#mermaid-diagram-auto">${esc(i18n.t('mermaidAutoTopologyTitle'))}</a></li>`);
			}
			if (!oh('rooms')) {
			...
			if (
				...
				((!uh('manual') && (mcUs.description || mcUs.contact || mcUs.notes)) ||
					(!uh('mermaid') &&
						((mcUs.mermaidDiagram && String(mcUs.mermaidDiagram).trim()) ||
							(mcUs.autoHostTopologyMermaid && String(mcUs.autoHostTopologyMermaid).trim()))))
			) {

 ...
 */
const RENDERER_VERSION = '2026.04.07.10';
...
class HtmlRenderer {
	...
	renderAdminHtml(docModel) {
		...
		if (docModel.aliases && docModel.aliases.length > 0) {
	...
	renderOnboardingHtml(docModel) {
		...
		body += this.renderOnboardingNotes(docModel.manualContext && docModel.manualContext.notes);
  ...
  if (localStorage.getItem('autodoc-dark') === '1') {
	...
	renderUserRoomsChapter(docModel) {
		...
		for (const room of visibleRooms) {
			...
			const summaryCls = note ? 'maint-details-summary maint-details-summary--room-note' : 'maint-details-summary';
...
<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">\n`;
  ...
  <span><strong>${esc(i18n.t('contact'))}:</strong> ${esc(contact)}</span>
		...
		// QR code section — CDN with copy-link fallback (always visible)
  ...
  <div id="qrcode" style="background:#fff;padding:8px;border-radius:6px;border:1px solid var(--border);display:inline-block"></div>
  <div id="qr-copy-btn" style="display:none;margin-top:6px">
...
`;
		...
		const hasManualContent = description || contact || (docModel.manualContext && docModel.manualContext.notes);
...
`;
	...
	renderOnboardingCapabilities(docModel) {
		...
		const iconMap = {
			licht: '💡', light: '💡', beleuchtung: '💡', lampe: '💡', lamp: '💡',
			rollladen: '🪟', shutter: '🪟', blind: '🪟', jalousie: '🪟', rollo: '🪟',
			heizung: '🌡️', heating: '🌡️', thermostat: '🌡️', klima: '❄️', climate: '❄️', ac: '❄️', kühlung: '❄️',
			steckdose: '🔌', socket: '🔌', outlet: '🔌', schalter: '🔌', switch: '🔌',
			sicherheit: '🔒', security: '🔒', alarm: '🔔', tür: '🚪', door: '🚪', fenster: '🪟',
			kamera: '📷', camera: '📷',
			musik: '🎵', media: '🎵', audio: '🎵', tv: '📺', fernseher: '📺',
			garten: '🌿', garden: '🌿', rasen: '🌿', bewässerung: '💧', irrigation: '💧',
			energie: '⚡', energy: '⚡', strom: '⚡', solar: '☀️',
			anwesenheit: '👤', presence: '👤', bewegung: '👁️', motion: '👁️',
			wetter: '🌤️', weather: '🌤️',
		};
		...
		if (roomsData.functions && roomsData.functions.length > 0) {
			...
			for (const room of (roomsData.rooms || [])) {
				for (const dev of (room.devices || [])) {
					...
					if (cat && cat !== '—') catMap.set(cat, (catMap.get(cat) || 0) + 1);
				}
			}
...
<li><a href="#adapter-instances">${esc(i18n.t('connectedSystems'))}</a></li>`;
			...
			links = `<li><a href="#manual-information">${esc(i18n.t('manualInformation'))}</a></li>
...
<li><a href="#aliases">${esc(i18n.t('aliases') || 'Aliase')}</a></li>
...
${system.hosts
	.map(h => {
		...
		if (res.sysTotalMb && res.sysFreeMb !== null) {
			...
			const tooltipText = esc(i18n.t('ramAdapterTooltip') || 'Summe aller ioBroker-Prozesse (js-controller + alle Adapter-Instanzen)');
			...
			const tooltipText = esc(i18n.t('ramHostTooltip') || 'Nur der ioBroker Host-Prozess (js-controller). Adapter-Instanzen sind separate Prozesse.');
...
`;
					for (const room of roomsWithDevices) {
						...
						const roomNote = docModel.manualContext && docModel.manualContext.rooms && docModel.manualContext.rooms[room.name];
						const summaryCls = roomNote ? 'maint-details-summary maint-details-summary--room-note' : 'maint-details-summary';
		...
		if (scriptsData.totalScripts === 0) {
			...
			const buildScriptRow = (script, active) => {
				...
				const rowTitle = hasSchedule
					...
					: '';
				const trAttr = hasSchedule ? ` class="table-row--has-schedule"${rowTitle ? ` title="${rowTitle}"` : ''}` : '';
				return `<tr${trAttr}>
  <td>${nameCell}</td>
  <td>${statusBadge}</td>
...
`;
	...
	renderUserDataChapter(userData) {
		...
		const filterId = 'userdata-filter';
...
<div class="adapter-filter-bar">
  <input type="search" id="${filterId}" placeholder="${esc(i18n.t('searchPlaceholder') || 'Suchen...')}" autocomplete="off" style="width:280px">
  <span id="${filterId}-count" style="font-size:12px;color:var(--text-faint)">${totalItems} ${esc(i18n.t('total') || 'gesamt')}</span>
</div>
...
(function(){
  ...
  var cnt = document.getElementById('${filterId}-count');
  if (!inp) return;
  inp.addEventListener('input', function(){
    var term = inp.value.trim().toLowerCase();
    var rows = document.querySelectorAll('#userdata-list tr[data-search]');
    var visible = 0;
    rows.forEach(function(r){
      var match = !term || r.getAttribute('data-search').includes(term);
      r.style.display = match ? '' : 'none';
      if (match) visible++;
    });
    if (cnt) cnt.textContent = (term ? visible + ' / ' : '') + ${totalItems} + ' ${esc(i18n.t('total') || 'gesamt')}';
    // Open all details when searching
    document.querySelectorAll('#userdata-list details').forEach(function(d){ if(term) d.open = true; });
  });
})();
<\/script>
<hr class="section-divider">
...
`;
			for (const item of items) {
				const targetHtml = item.readTarget === item.writeTarget || !item.writeTarget
				...
				const searchStr = (item.name + ' ' + (item.desc || '') + ' ' + (item.readTarget || '') + ' ' + (item.role || '')).toLowerCase();
...
(function(){
  ...
  inp.addEventListener('input', function(){
    var term = inp.value.trim().toLowerCase();
    var rows = document.querySelectorAll('#alias-list tr[data-search]');
    var visible = 0;
    rows.forEach(function(r){
      var match = !term || r.getAttribute('data-search').includes(term);
      r.style.display = match ? '' : 'none';
    ...
    if (cnt) cnt.textContent = (term ? visible + ' / ' : '') + ${totalItems} + ' ${esc(i18n.t('total') || 'gesamt')}';
    document.querySelectorAll('#alias-list details').forEach(function(d){ if(term) d.open = true; });
  });
})();
<\/script>
<hr class="section-divider">
...
<p style="font-size:13px;color:var(--text-faint);margin-bottom:10px">${esc(i18n.t('scoreDesc') || 'Der Score zeigt wie vollständig die Dokumentation ist: alle Skripte beschriftet + alle Instanzen aktiv = 100 %.')}</p>
...
<summary class="maint-details-summary maint-details-summary--attention">${swdLabel}</summary>
...
${(() => {
	const loc = system.location;
	if (!loc || !loc.activeRepo) return '';
	const repos = loc.activeRepo.toLowerCase();
	const isBeta = repos.includes('beta') || repos.includes('latest');
	const badgeStyle = isBeta
		? 'background:#fff3cd;color:#856404;border:1px solid #ffc107'
		: 'background:#d4edda;color:#155724;border:1px solid #c3e6cb';
	const badge = `<span style="display:inline-block;padding:1px 8px;border-radius:10px;font-size:12px;font-weight:600;${badgeStyle}">${esc(loc.activeRepo)}</span>`;
	return `  <dt>${esc(i18n.t('activeRepo') || 'Repository')}</dt><dd>${badge}</dd>`;
})()}
</dl>

 ...
 */
const RENDERER_VERSION = '2026.04.07.01';
...
<div class="search-wrap">
  ...
  <div id="search-hint" style="font-size:11px;color:#666;margin-top:3px;display:none">${esc(this.i18n.t('searchHint') || '↑↓ navigieren · Esc = löschen')}</div>
</div>
${nav}
...
`;
		...
		// Location + Timezone + Repository
		const loc = system.location;
		if (loc && (loc.city || loc.country || loc.timezone || loc.activeRepo)) {
			const locParts = [loc.city, loc.country].filter(Boolean).join(', ');

			// Repository badge: beta gets a warning colour, stable gets green
			let repoBadge = '';
			if (loc.activeRepo) {
				...
				repoBadge = ` <span style="display:inline-block;padding:1px 8px;border-radius:10px;font-size:12px;font-weight:600;${badgeStyle}">${esc(loc.activeRepo)}</span>`;
			}
...
${loc.activeRepo ? `  <dt>${esc(i18n.t('activeRepo') || 'Repository')}</dt><dd>${repoBadge}</dd>` : ''}
</dl>
`;
		}

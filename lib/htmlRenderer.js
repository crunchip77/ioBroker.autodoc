...
class HtmlRenderer {
	...
	renderAdminHtml(docModel) {
		...
		body += this.renderTroubleshooting(docModel, PROFILE_ADMIN);
...
<table>
<thead><tr><th>${esc(i18n.t('name'))}</th><th>${esc(i18n.t('platform'))}</th><th>${esc(i18n.t('version'))}</th><th>${esc(i18n.t('adapters'))}</th></tr></thead>
<tbody>
...
<table>
<thead><tr><th>${esc(i18n.t('name'))}</th><th>${esc(i18n.t('description'))}</th><th>${esc(i18n.t('totalInstances'))}</th><th>${esc(i18n.t('enabledInstances'))}</th></tr></thead>
<tbody>
`;
			for (const adapter of adapters.adapters) {
				...
				// Build meta badges for admin profile
				const metaBadges = [];
						...
						adapter.dataSource === 'push'
							...
							: adapter.dataSource === 'poll'
								...
								: adapter.dataSource === 'assumption'
									? 'Annahme'
									: esc(adapter.dataSource);
					metaBadges.push(`<span class="badge badge-meta">${dsLabel}</span>`);
				}
				if (adapter.tier) {
					const stars = '⭐'.repeat(adapter.tier);
					metaBadges.push(`<span class="badge badge-meta">${stars} Tier ${esc(adapter.tier)}</span>`);
				}
				const metaHtml =
					metaBadges.length > 0 ? `<br><span class="adapter-meta">${metaBadges.join(' ')}</span>` : '';

				// manualContext hint for admin (show as note in desc column)
				const manualNote =
					docModel.manualContext &&
					docModel.manualContext.adapters &&
					docModel.manualContext.adapters[adapter.name]
...
`;
		...
		if (m.instancesWithoutRoom.length > 0) {
			html += `<h3>${esc(i18n.t('instancesWithoutRoom'))}</h3>
<table><thead><tr><th>ID</th><th>${esc(i18n.t('name'))}</th></tr></thead><tbody>
${m.instancesWithoutRoom.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
</tbody></table>\n`;
		}
...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
	...
	renderTroubleshooting(docModel, profile) {
		...
		let html = `<h2 id="troubleshooting">${esc(i18n.t('troubleshooting'))}</h2>
<h3>${esc(i18n.t('commonIssues'))}</h3>
<h4>${esc(i18n.t('adapterNotResponding'))}</h4>
<ul class="content-list">
  <li>${esc(i18n.t('troubleshootCheck1'))}</li>
  <li>${i18n.t('troubleshootCheck2', system.primaryHost.name)}</li>
  <li>${esc(i18n.t('troubleshootCheck3'))}</li>
</ul>
<h4>${esc(i18n.t('stateObjectsNotUpdating'))}</h4>
<ul class="content-list">
  <li>${esc(i18n.t('troubleshootState1'))}</li>
  <li>${esc(i18n.t('troubleshootState2'))}</li>
  <li>${esc(i18n.t('troubleshootState3'))}</li>
</ul>
`;

		if (profile === PROFILE_ADMIN) {
			html += `<h4>${esc(i18n.t('collectorStatus'))}</h4>
<dl class="meta">
  <dt>${esc(i18n.t('instancesDetected'))}</dt><dd>${esc(docModel.system.statistics.instanceCount)}</dd>
  <dt>${esc(i18n.t('stateObjectsScanned'))}</dt><dd>${esc(docModel.appendices.stateSummary.total)}</dd>

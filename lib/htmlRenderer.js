...
<table>
<thead><tr><th>${esc(i18n.t('name'))}</th><th>${esc(i18n.t('platform'))}</th><th>${esc(i18n.t('version'))}</th></tr></thead>
<tbody>
${system.hosts.map(h => `<tr><td>${esc(h.name)}</td><td>${esc(h.platform)}</td><td>${esc(h.version)}</td></tr>`).join('\n')}
</tbody>
</table>
`;
		}

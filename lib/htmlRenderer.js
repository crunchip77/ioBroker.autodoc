 ...
 */
const RENDERER_VERSION = '2026.04.18.5';
...
class HtmlRenderer {
	...
	renderCustomSectionsBlock(docModel, profile) {
		...
		for (const s of sections) {
			const inner = md.render(s.bodyMarkdown);
	...
	renderAdminHtml(docModel) {
		...
		// Manual context (project description, contact, notes) shown early — before technical chapters
		if (
			...
			(docModel.manualContext.description ||
				...
				docModel.manualContext.notes ||
				docModel.manualContext.guestHelpNote ||
				docModel.manualContext.homeRoutinesNote)
		) {
			body += this.renderManualContext(docModel.manualContext, PROFILE_ADMIN);
		}
		...
		if (
			...
			body += this.renderScheduleObjectsSection(docModel);
		}
		...
		if (!this.adminChapterHidden(docModel, 'aliases') && docModel.aliases && docModel.aliases.length > 0) {
			body += this.renderAliasChapter(docModel.aliases);
		}
		...
		body += this.renderCustomSectionsBlock(docModel, PROFILE_ADMIN);
		if (!this.adminChapterHidden(docModel, 'changelog') && docModel.changelog && docModel.changelog.length > 0) {
	...
	renderUserHtml(docModel) {
		...
		if (!this.userChapterHidden(docModel, 'guestHelp')) {
			body += this._renderGuestHelpChapter(docModel.manualContext);
		}
	...
	renderOnboardingHtml(docModel, renderOptions) {
		...
		// 2. Tips & Notes — always shown (friendly fallback when empty)
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  main { flex: 1; padding: 32px 40px; max-width: min(1280px, calc(100vw - 280px)); }
...
`;
		...
		const hasManualContent =
			...
			(mc &&
				(mc.notes ||
					(mc.guestHelpNote && String(mc.guestHelpNote).trim()) ||
...
`;
  ...
  ${note ? `<div class="manual-context-box">${esc(note)}</div>` : ''}
	...
	/**
	 * Guest/family: manual “help & emergencies” block (plain language).
	 *
	 * @param {object} [manualContext]
	 * @returns {string}
	 */
	_renderGuestHelpChapter(manualContext) {
		const note = manualContext && manualContext.guestHelpNote;
		if (!note || !String(note).trim()) {
			return '';
		}
		const i18n = this.i18n;
		return `<h2 id="guest-help">${esc(i18n.t('guestHelpTitle'))}</h2>
...
<hr class="section-divider">\n`;
	...
	renderNav(profile, docModel) {
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			if (!oh('guestHelp') && mcOb && mcOb.guestHelpNote && String(mcOb.guestHelpNote).trim()) {
			...
			if (!uh('guestHelp') && mcUs && mcUs.guestHelpNote && String(mcUs.guestHelpNote).trim()) {
			...
			const lines = [];
			if (!h('system')) {
				lines.push(`<li><a href="#system-overview">${esc(i18n.t('systemOverview'))}</a></li>`);
			}
			if (!h('manual') && hasManual) {
				lines.push(`<li><a href="#manual-information">${esc(i18n.t('manualInformation'))}</a></li>`);
			}
			if (!h('adapters')) {
				lines.push(`<li><a href="#adapter-instances">${esc(i18n.t('adapterInstances'))}</a></li>`);
			}
			if (!h('rooms')) {
				lines.push(`<li><a href="#rooms-and-functions">${esc(i18n.t('roomsAndFunctions'))}</a></li>`);
			}
			if (!h('scripts')) {
				lines.push(`<li><a href="#scripts">${esc(i18n.t('scripts'))}</a></li>`);
			}
			if (!h('schedule') && dm.scheduleObjects && dm.scheduleObjects.length > 0) {
				lines.push(`<li><a href="#schedule-objects">${esc(i18n.t('scheduleTypeObjects'))}</a></li>`);
			}
			if (!h('userdata')) {
				lines.push(
			...
			if (!h('aliases')) {
				lines.push(`<li><a href="#aliases">${esc(i18n.t('aliases') || 'Aliase')}</a></li>`);
			}
			if (!h('maintenance')) {
				lines.push(`<li><a href="#maintenance">${esc(i18n.t('maintenance'))}</a></li>`);
			}
			if (!h('diagnosis')) {
				lines.push(`<li><a href="#diagnosis">${esc(i18n.t('diagnosis'))}</a></li>`);
			}
			if (!h('troubleshooting')) {
				lines.push(`<li><a href="#troubleshooting">${esc(i18n.t('troubleshooting'))}</a></li>`);
			}
			lines.push(this.renderCustomSectionNavItems(PROFILE_ADMIN, docModel));
			if (!h('changelog')) {
				lines.push(`<li><a href="#changelog">${esc(i18n.t('changelog'))}</a></li>`);
			}
			if (!h('appendices')) {
				lines.push(`<li><a href="#appendices">${esc(i18n.t('appendices'))}</a></li>`);
			}
			links = lines.filter(Boolean).join('\n');
		}
...
<table>
<thead><tr><th>${esc(i18n.t('name'))}</th><th>${esc(i18n.t('platform'))}</th><th>${esc(i18n.t('version'))}</th><th>${esc(i18n.t('nodeVersion'))}</th><th>${esc(i18n.t('npmVersion'))}</th><th>RAM</th><th>CPU</th><th>${esc(i18n.t('uptime') || 'Laufzeit')}</th></tr></thead>
<tbody>
${system.hosts
	.map(h => {
...
`;
	...
	renderHostDistribution(docModel) {
		...
		for (const hostName of Object.keys(instancesByHost).sort()) {
			...
			const osLabel = info.osType ? esc(info.osType) : info.platform ? esc(info.platform) : '';
			const archLabel = info.osArch ? esc(info.osArch) : '';
  ...
  <div style="margin-bottom:8px;display:flex;gap:6px;flex-wrap:wrap">
    ...
    ${osLabel ? `<span class="badge badge-meta" style="font-size:11px">${osLabel}${archLabel ? ` ${archLabel}` : ''}</span>` : ''}
  </div>
...
`;
  ...
  <th>${esc(i18n.t('scriptDescription'))}</th>
				...
				const renderFolderBlock = (scriptsInFolder, folderKey, folder, tbodyId, active, openDetails) => {
					...
					const globalHint = isGlobalFolderKey(folderKey)
						...
						: '';
					const openAttr = openDetails ? ' open' : '';
					return `<details class="script-folder-group"${openAttr}>
...
`;
				...
				for (const g of groupedActive) {
					...
					const openFirstGlobal = isGlobalFolderKey(g.folderKey);
					html += renderFolderBlock(g.scripts, g.folderKey, g.folder, tbodyId, true, openFirstGlobal);
				}
...
`;
					...
					for (const g of groupedInactive) {
						...
						html += renderFolderBlock(
							g.scripts,
							g.folderKey,
							g.folder,
							tbodyId,
							false,
							isGlobalFolderKey(g.folderKey),
						);
					}
...
<section id="autodoc-userdata-section" data-autodoc-total="${totalItems}" data-autodoc-total-suffix="${totalSuffix}">
<div class="adapter-filter-bar">
...
<section id="autodoc-alias-section" data-autodoc-total="${totalItems}" data-autodoc-total-suffix="${totalSuffix}">
<div class="adapter-filter-bar">
...
<summary class="maint-details-summary maint-details-summary--attention">${diLabel}</summary>
...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
	...
	renderDiagnosis(docModel) {
		...
		if (hostRes.sysTotalMb && hostRes.sysFreeMb !== null) {
			...
			ramForumText = `~${hostRes.adapterTotalMb} MB (alle Adapter)`;
		...
		// Forum copy data block — plain text representation of all fields
		const forumData = {
			instances: `${stats.instanceCount} (${stats.enabledInstanceCount} aktiv, ${stats.disabledInstanceCount} inaktiv)`,
			stateObjects: `${appendices.stateSummary.total} (${appendices.stateSummary.writable} schreibbar, ${appendices.stateSummary.readonly} nur lesbar)`,
...
<script type="application/json" id="forum-data">${JSON.stringify(forumData)}</script>
<dl class="meta">
...
function copyForumData(btn) {
  try {
    var d = JSON.parse(document.getElementById('forum-data').textContent);
    var fence = '\x60\x60\x60';
    var lines = [
      fence,
      'Erkannte Instanzen:    ' + d.instances,
      'Plattform:             ' + d.platform,
      'js-controller:         ' + d.jsController,
      'Node.js:               ' + d.nodejs,
      'npm:                   ' + d.npm,
      'RAM:                   ' + d.ram,
      'CPU:                   ' + d.cpu,
...
`;
  ...
  <ul style="margin:4px 0 0 16px;font-size:12px;color:var(--text-faint)">\n`;
				for (const change of entry.changes) {
					card += `    <li><span style="color:var(--link);font-weight:600">${esc(change.type)}</span> ${esc(change.message)}</li>\n`;
				}
				card += `  </ul>\n  </details>`;
			}
			card += `\n</div>\n`;

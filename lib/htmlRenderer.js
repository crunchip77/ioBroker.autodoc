...
const {
	buildDocChangeSinceLastRun,
	localizeCompareSummary,
	shouldShowUserFriendlyDocChange,
} = require('./docChangeFormat');
const { formatScriptTriggerBreakdown } = require('./scriptAutomationStats');
 ...
 */
const RENDERER_VERSION = '2026.09.22.9';
...
/**
 * Whether this instance has at least one **automatic** documentation generation path enabled
 * (same rules as `main.js` scheduling).
 *
 * Display logic (Admin HTML → `renderTroubleshooting`):
 * - The subsection `tsDocNotGenerated` is emitted **only** when this returns **false**
 *   (all off: start ✗, adapter-change events ✗, hours interval 0/empty).
 * - If **any** of the three is on, that subsection is **omitted** — the exported HTML
 *   then reflects a config where “missing checkboxes” is not the cause of “no auto run”.
 *
 * @param {object} [config] Adapter `native` config at **export** time
 * @returns {boolean}
 */
function hasAutomaticDocGenerationTrigger(config) {
	const c = config || {};
	if (c.autoGenerateOnStart === true) {
		return true;
	}
	if (c.autoGenerateOnEvents === true) {
		return true;
	}
	const n = Number(c.autoGenerateInterval);
	return Number.isFinite(n) && n > 0;
}

/** Max. JavaScript scripts listed by name when `onboardingGuestShowScriptNames` is enabled (legacy / opt-in). */
const ONBOARDING_AUTOMATIONS_MAX = 5;

const { groupScriptsByFolder, isGlobalFolderKey } = require('./scriptGroups');
const { getMermaidClientThemeMatrix } = require('./mermaidAutodocPalette');
...
const { guestHelpChapterHasContent, publicLinksRelevantForProfile } = require('./guestHelpContent');
...
class HtmlRenderer {
	...
	getMarkdownIt() {
		if (!this._md) {
			...
			md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
				...
				if (aIndex >= 0 && attrs) {
					const href = String(attrs[aIndex][1]);
	 ...
	 * When `@mermaid-js/mermaid-cli` is installed (optional dependency), Mermaid blocks are inlined as SVG for offline/PDF-friendly HTML.
	 *
	 * @param {object} docModel Document model
	 ...
	 * @returns {Promise<{admin: string, user: string, onboarding: string}>} HTML for all profiles
	 */
	async renderAllHtml(docModel, renderOptions) {
		const { embedMermaidDiagramsInHtml } = require('./mermaidServerSvg');
		const colorScheme = parseHtmlColorScheme(this.adapter.config.htmlColorScheme);
		const embedOpts = {
			colorScheme,
			themePreset: parseHtmlThemePreset(this.adapter.config.htmlThemePreset),
			log: this.adapter.log,
		};

		let admin = this.renderAdminHtml(docModel);
		let user = this.renderUserHtml(docModel);
		let onboarding = this.renderOnboardingHtml(docModel, renderOptions);

		admin = await embedMermaidDiagramsInHtml(admin, embedOpts);
		user = await embedMermaidDiagramsInHtml(user, embedOpts);
		onboarding = await embedMermaidDiagramsInHtml(onboarding, embedOpts);

		return { admin, user, onboarding };
	}
	...
	renderAdminChapterBodyKey(docModel, key) {
		...
		const hasManual =
			...
			(mc.description ||
				mc.contact ||
				mc.notes ||
				(mc.mermaidDiagram && String(mc.mermaidDiagram).trim()) ||
				(mc.autoHostTopologyMermaid && String(mc.autoHostTopologyMermaid).trim()) ||
				(mc.homeRoutinesNote && String(mc.homeRoutinesNote).trim()) ||
				(mc.ownerPlaybookNote && String(mc.ownerPlaybookNote).trim()) ||
		...
		switch (key) {
			case 'manual': {
				...
				return this.renderManualContext(mc, PROFILE_ADMIN, docModel);
			}
			case 'system': {
			...
			case 'automationOverview': {
				if (
					this.adminChapterHidden(docModel, 'automationOverview') ||
					!docModel.automationOverview ||
					docModel.automationOverview.isEmpty
				) {
					return '';
				}
				return this.renderAutomationOverviewChapter(docModel);
			}
			case 'scripts': {
	...
	buildAdminNavFragmentForKey(docModel, key) {
		...
		switch (key) {
			...
			case 'automationOverview': {
				if (h('automationOverview') || !dm.automationOverview || dm.automationOverview.isEmpty) {
					return '';
				}
				return `<li><a href="#automation-overview">${esc(i18n.t('automationOverviewTitle'))}</a></li>`;
			}
			case 'scripts': {
	 ...
	 */
	/**
	 * Dispatch one User-profile chapter body by key.
	 * Note: 'manual' renders both the manual context AND the mermaid diagram; 'mermaid' is a no-op here.
	 *
	 * @param {object} docModel
	 * @param {string} key Chapter id from {@link USER_HTML_CHAPTER_KEYS}
	 * @returns {string} HTML fragment
	 */
	renderUserChapterBodyKey(docModel, key) {
		const h = k => this.userChapterHidden(docModel, k);
		const uMc = docModel.manualContext;
		switch (key) {
			case 'manual':
			case 'mermaid': {
				if (key === 'mermaid') {
				...
				const hasCore = uMc && !h('manual') && (uMc.description || uMc.contact || uMc.notes);
				const hasMermaid = uMc && !h('mermaid') && uMc.mermaidDiagram && String(uMc.mermaidDiagram).trim();
				const hasMermaidAuto =
					uMc &&
					!h('mermaidAuto') &&
					uMc.autoHostTopologyMermaid &&
					String(uMc.autoHostTopologyMermaid).trim();
				if (hasCore || hasMermaid || hasMermaidAuto) {
					return this.renderManualContext(uMc, PROFILE_USER, docModel);
				}
				return '';
			}
			case 'ai': {
				if (!docModel.ai?.user || h('ai')) {
					return '';
				}
				return this.renderAiSection(docModel.ai.user, {
			...
			case 'guestHelp':
				...
				return this._renderGuestHelpChapter(docModel, PROFILE_USER);
			case 'atAGlance':
				if (h('atAGlance') || !(docModel.quickStart && docModel.quickStart.hasContent)) {
					return '';
				}
				return this.renderUserAtAGlance(docModel);
			case 'rooms':
				if (h('rooms')) {
					return '';
				}
				return this.renderUserRoomsChapter(docModel);
			case 'scripts':
				if (h('scripts')) {
					return '';
				}
				return this.renderUserScriptsChapter(docModel);
			case 'routines':
				if (h('routines')) {
					return '';
				}
				return this._renderHomeRoutinesChapter(docModel.manualContext);
			case 'ownerPlaybook':
				if (h('ownerPlaybook')) {
					return '';
				}
				return this._renderOwnerPlaybookChapter(docModel.manualContext);
			case 'adapters':
				if (h('adapters')) {
					return '';
				}
				return this.renderUserAdaptersChapter(docModel);
			case 'custom':
				return this.renderCustomSectionsBlock(docModel, PROFILE_USER);
			default:
				return '';
		}
	}
	...
	renderUserHtml(docModel) {
		...
		let body = this.renderHeader(docModel, PROFILE_USER);
		body += this.renderUserSinceLastDocNoticeHtml(docModel);
		const order = (docModel && docModel.userChapterOrder) || USER_HTML_CHAPTER_KEYS;
		for (const key of order) {
			body += this.renderUserChapterBodyKey(docModel, key);
		}
		return this.wrapPage(title, nav, body);
	}

	/**
	 * Short User-profile notice when inventory deltas exist since last snapshot (non-technical).
	 *
	 * @param {object} docModel Document model
	 * @returns {string} HTML fragment or empty
	 */
	renderUserSinceLastDocNoticeHtml(docModel) {
		if (!shouldShowUserFriendlyDocChange(docModel.docChangeSinceLastRun)) {
			return '';
		}
		const i18n = this.i18n;
		return `<div class="note-box" id="user-doc-change-since-last"><p style="margin:0">${esc(i18n.t('userDocChangeSinceLastPlain'))}</p></div>\n`;
	}
	 ...
	 */
	/**
	 * Dispatch one Onboarding-profile chapter body by key.
	 * 'welcome' is always rendered first outside the order loop and returns '' here.
	 *
	 * @param {object} docModel
	 * @param {string} key Chapter id from {@link ONBOARDING_HTML_CHAPTER_KEYS}
	 * @param {object} [_renderOptions] Unused; reserved for future url/qr options
	 * @returns {string} HTML fragment
	 */
	renderOnboardingChapterBodyKey(docModel, key, _renderOptions) {
		const h = k => this.onboardingChapterHidden(docModel, k);
		switch (key) {
			case 'welcome':
				return ''; // always rendered before the order loop
			case 'quickstart':
				if (h('quickstart')) {
					return '';
				}
			...
			case 'tips':
				return this.renderOnboardingNotes(docModel); // always shown
			case 'guestHelp':
				if (h('guestHelp')) {
					return '';
				}
				return this._renderGuestHelpChapter(docModel, PROFILE_ONBOARDING);
			case 'stats':
				if (h('stats')) {
					return '';
				}
				return this.renderOnboardingStats(docModel);
			case 'ai':
				if (!docModel.ai?.onboarding || h('ai')) {
					return '';
				}
				return this.renderAiSection(docModel.ai.onboarding, {
			...
			case 'capabilities':
				...
				return this.renderOnboardingCapabilities(docModel);
			case 'mermaid':
				return ''; // rendered as part of the manual/welcome block elsewhere
			case 'rooms':
				if (h('rooms')) {
					return '';
				}
				return this.renderOnboardingRooms(docModel);
			case 'routines':
				if (h('routines')) {
					return '';
				}
			...
			case 'automations':
				...
				return this.renderOnboardingAutomations(docModel);
			case 'adapters':
				if (h('adapters')) {
					return '';
				}
				return this.renderOnboardingAdapters(docModel);
			case 'custom':
				return this.renderCustomSectionsBlock(docModel, PROFILE_ONBOARDING);
			case 'hint':
				if (h('hint')) {
					return '';
				}
				return this.renderOnboardingHint(docModel);
			default:
				return '';
		}
	}
	...
	renderOnboardingHtml(docModel, renderOptions) {
		...
		// Welcome (incl. QR) is always first regardless of order
		let body = this.renderOnboardingWelcome(docModel, renderOptions);
		const order = (docModel && docModel.onboardingChapterOrder) || ONBOARDING_HTML_CHAPTER_KEYS;
		for (const key of order) {
			body += this.renderOnboardingChapterBodyKey(docModel, key, renderOptions);
		}
		return this.wrapPage(title, nav, body);
	}
...
`;
		...
		return `<!DOCTYPE html>
<html lang="en"${htmlClassAttr} data-autodoc-preset="${esc(themePreset)}">
<head>
<meta charset="UTF-8">
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .adapter-section-summary { cursor: pointer; padding: 8px 12px; user-select: none; list-style: none; font-weight: 600; font-size: 15px; color: var(--text); display: inline-flex; align-items: center; gap: 6px; border: 1px solid var(--border); border-radius: 6px; background: var(--surface); transition: background 0.15s, border-color 0.15s; }
  .adapter-section-summary::-webkit-details-marker { display: none; }
  .adapter-section-summary::before { content: '\\25B6'; font-size: 10px; color: var(--disclosure-chevron); transition: transform 0.15s, color 0.15s; display: inline-block; }
  .adapter-section-summary:hover::before { color: var(--link); }
  #adapter-section[open] > .adapter-section-summary::before { transform: rotate(90deg); }
  .adapter-section-summary:hover { border-color: var(--link, #0066cc); background: var(--surface-alt, rgba(0,102,204,0.05)); }
  .adapter-section-summary .adapter-section-counts { font-size: 12px; font-weight: 400; color: var(--text-faint); }
  ...
  .adapter-card-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 10px; margin: 4px 0 16px; }
  .adapter-card-item { background: var(--surface); border: 1px solid var(--border); border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 6px; min-width: 0; transition: box-shadow 0.15s; }
  .adapter-card-item:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.10); }
  .adapter-card-item.filter-hidden { display: none; }
  .adapter-card-item.table-row--manual-note { border-left: 3px solid var(--accent, #0066cc); }
  .aci-header { display: flex; flex-direction: column; gap: 1px; }
  .aci-header strong { font-size: 14px; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .aci-header small { font-size: 11px; color: var(--text-faint); }
  .aci-desc { font-size: 12px; color: var(--text-faint); line-height: 1.4; }
  .aci-meta { display: flex; flex-wrap: wrap; align-items: center; gap: 4px; margin-top: 2px; }
  .aci-counts { font-size: 11px; color: var(--text-faint); margin-left: auto; white-space: nowrap; }
  .aci-instances summary { font-size: 11px; color: var(--text-faint); cursor: pointer; user-select: none; list-style: none; padding: 2px 0; }
  .aci-instances summary::-webkit-details-marker { display: none; }
  .aci-instances summary::before { content: '\\25B6\\00A0'; font-size: 9px; color: var(--disclosure-chevron); display: inline-block; transition: transform 0.15s, color 0.15s; }
  .aci-instances summary:hover::before { color: var(--link); }
  .aci-instances[open] summary::before { transform: rotate(90deg); }
  .aci-instances-content { margin-top: 4px; padding-top: 6px; border-top: 1px solid var(--border); display: flex; flex-direction: column; gap: 2px; }
  .aci-inst-row { font-size: 11px; color: var(--text-faint); display: flex; flex-wrap: wrap; align-items: center; gap: 4px; }
  .aci-manual-note { font-size: 12px; color: var(--text); background: var(--surface-alt, rgba(0,102,204,0.06)); border-radius: 4px; padding: 4px 8px; margin-top: 2px; }
  ...
  .maint-details-summary::marker { content: ''; }
  .maint-details-summary::before { content: '▶\\00a0'; font-size: 10px; line-height: 1; color: var(--disclosure-chevron); display: inline-block; width: 1em; transition: transform 0.15s ease, color 0.15s ease; vertical-align: baseline; }
  details[open] > .maint-details-summary::before { transform: rotate(90deg); }
  .maint-details-summary:hover { color: var(--link); }
  .maint-details-summary:hover::before { color: var(--link); }
  /* Manual hint accent stays on boxes/rows — chevron colour stays uniform */
  .maint-details-summary--room-note:hover { color: var(--text); }
  .mermaid-wrap { margin: 16px 0; padding: 12px 14px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; overflow-x: auto; }
  .mermaid-wrap pre.mermaid { margin: 0; background: transparent; white-space: pre-wrap; word-break: break-word; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 13px; color: var(--text); }
  .mermaid-svg-embedded svg { max-width: 100%; height: auto; display: block; }
  ...
  .maint-details-summary--attention { color: var(--text-muted); }
...
(function () {
  ...
  var adapterSection = document.getElementById('adapter-section');
  ...
  function applyAdapterFilter() {
    ...
    var enabledCards = Array.from(document.querySelectorAll('#adapter-enabled-body .adapter-card-item'));
    var disabledCards = Array.from(document.querySelectorAll('#adapter-disabled-body .adapter-card-item'));
    var allCards = enabledCards.concat(disabledCards);

    if (!term) {
      allCards.forEach(function (c) { c.classList.remove('filter-hidden'); });
    ...
    // auto-open outer section when user starts filtering
    if (adapterSection && !adapterSection.open) adapterSection.open = true;

    var visible = 0;
    var disabledHasMatch = false;
    allCards.forEach(function (c) {
      var match = c.textContent.toLowerCase().indexOf(term) !== -1;
      c.classList.toggle('filter-hidden', !match);
      if (match) {
        visible++;
        if (disabledCards.indexOf(c) !== -1) disabledHasMatch = true;
      }
    });
    ...
    if (countEl) countEl.textContent = visible + ' / ' + allCards.length;

    var enabledVisible = enabledCards.filter(function (c) { return !c.classList.contains('filter-hidden'); }).length;
    var disabledVisible = disabledCards.filter(function (c) { return !c.classList.contains('filter-hidden'); }).length;
    var en = document.getElementById('adapter-enabled-noresults');
    var dn = document.getElementById('adapter-disabled-noresults');
...
function toggleDark() {
  ...
  if (typeof window.rerenderMermaid === 'function') window.rerenderMermaid();
}
(function () {
...
// Keep marked sections collapsed on load (browser restore / filter scripts must not leave them open).
(function () {
  function closeDefaultSections() {
    document.querySelectorAll('details[data-autodoc-default-closed]').forEach(function (d) {
      d.removeAttribute('open');
    });
  }
  closeDefaultSections();
  document.addEventListener('DOMContentLoaded', closeDefaultSections);
  window.addEventListener('pageshow', closeDefaultSections);
})();
</script>
${
	/<pre class="mermaid"/.test(body)
		? `
...
(function () {
  ...
  var AUTODOC_MERMAID_THEME = ${JSON.stringify(getMermaidClientThemeMatrix())};
  function themeVars() {
    var preset = document.documentElement.getAttribute('data-autodoc-preset') || 'default';
    var bucket = AUTODOC_MERMAID_THEME[preset] || AUTODOC_MERMAID_THEME.default;
    return document.body.classList.contains('dark') ? bucket.dark : bucket.light;
  }
  function initAndRun(nodeList) {
    if (typeof mermaid === 'undefined') return;
    try {
      mermaid.initialize({ startOnLoad: false, securityLevel: 'strict', theme: 'base', themeVariables: themeVars() });
      mermaid.run({ nodes: Array.prototype.slice.call(nodeList) });
    } catch (e) { console.warn('Mermaid:', e); }
  }
  function runM() {
    nodes.forEach(function (n) {
      if (!n.getAttribute('data-mermaid-src')) n.setAttribute('data-mermaid-src', n.textContent);
    });
    initAndRun(nodes);
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', runM);
  } else {
    runM();
  }
})();
	...
	renderUserRoomsChapter(docModel) {
		...
		for (const room of visibleRooms) {
			...
			const devCount = room.deviceCount != null ? room.deviceCount : devices.length;
			const note =
				docModel.manualContext && docModel.manualContext.rooms && docModel.manualContext.rooms[room.name];
			const roomSummary = `${esc(room.name)} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${devCount} ${esc(i18n.t('roomDevices'))})</span>`;
...
`;
			...
			if (devCount === 0) {
				...
				for (const dev of devices) {
					html += this._renderDeviceCard(dev);
				}
				html += `</div>\n`;
			}
			html += `</details>\n`;
		...
		// Functions — same device-card pattern as rooms (enum.functions stay visible even when empty)
		if (roomsData.functions && roomsData.functions.length > 0) {
			html += `<h3 style="margin-top:28px">${esc(i18n.t('functions'))}</h3>
<details class="autodoc-collapsible-section" data-autodoc-default-closed="1">
...
<div style="margin-top:8px">\n`;
			html += this._renderNamedDeviceGroupDetails(roomsData.functions, i18n, 'noDevicesInFunction');
			html += `</div>\n</details>\n`;
		}
	...
	/**
	 * Trigger-mix summary for User/Onboarding when script names or desc are omitted (privacy / no common.desc).
	 *
	 * @param {object} docModel Document model
	 * @param {'user'|'onboarding'} profile
	 * @returns {string} HTML paragraph(s) or empty
	 */
	_renderFamilyAutomationsSummary(docModel, profile) {
		const i18n = this.i18n;
		const enabledTotal =
			docModel.automationOverview &&
			docModel.automationOverview.scriptStats &&
			docModel.automationOverview.scriptStats.enabledTotal != null
				? docModel.automationOverview.scriptStats.enabledTotal
				: ((docModel.scripts && docModel.scripts.scripts) || []).filter(s => s.enabled).length;
		if (enabledTotal <= 0) {
			return '';
		}
		const bodyKey = profile === 'onboarding' ? 'onboardingAutomationsSummaryBody' : 'userAutomationsSummaryBody';
		const mix = this._renderFamilyAutomationsTriggerMix(docModel, true);
		let html = `<div class="manual-context-box" style="margin-bottom:12px">
<p style="font-size:14px;color:var(--text);line-height:1.55;margin:0 0 ${mix ? '8px' : '0'}">${esc(
			i18n.t(bodyKey, enabledTotal),
		)}</p>
${mix}
</div>\n`;
		return html;
	...
	/**
	 * @param {object} docModel Document model
	 * @param {boolean} [insideSummaryBox] When true, remove outer paragraph margin for info boxes.
	 * @returns {string} HTML paragraph or empty
	 */
	_renderFamilyAutomationsTriggerMix(docModel, insideSummaryBox) {
		const i18n = this.i18n;
		const stats = docModel.automationOverview && docModel.automationOverview.scriptStats;
		const breakdown =
			stats && stats.enabledTotal
				? formatScriptTriggerBreakdown(stats, (key, ...args) => i18n.t(key, ...args))
				: '';
		if (!breakdown) {
			return '';
		}
		const margin = insideSummaryBox ? '0' : '0 0 12px';
		return `<p style="font-size:13px;color:var(--text-muted);line-height:1.5;margin:${margin}">${esc(
			i18n.t('familyAutomationsTriggerMix'),
		)} ${esc(breakdown)}.</p>\n`;
	}

	/**
	 * Collapsible name + device grid (User rooms/functions share this layout).
	 *
	 * @param {Array<{ name: string, devices?: object[], deviceCount?: number }>} groups Room or function rows from documentModel
	 * @param {object} i18n I18n instance
	 * @param {string} emptyDevicesKey i18n key when no devices resolved
	 * @returns {string}
	 */
	_renderNamedDeviceGroupDetails(groups, i18n, emptyDevicesKey) {
		let html = '';
		for (const group of groups) {
			const devices = group.devices || [];
			const devCount = group.deviceCount != null ? group.deviceCount : devices.length;
			const summary = `${esc(group.name)} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${devCount} ${esc(
				i18n.t('roomDevices'),
			)})</span>`;
			html += `<details style="margin-bottom:8px">
<summary class="maint-details-summary">${summary}</summary>
`;
			if (devCount === 0) {
				html += `<p style="color:var(--text-faint);font-size:14px;margin-top:6px"><em>${esc(i18n.t(emptyDevicesKey))}</em></p>\n`;
			} else {
				html += `<div class="device-grid" style="margin-top:6px">\n`;
				for (const dev of devices) {
					html += this._renderDeviceCard(dev);
	...
	/**
	 * Admin table: resolved devices with category and OID (rooms + functions).
	 *
	 * @param {object} i18n I18n instance
	 * @param {Array<object>} devices Device rows from documentModel
	 * @returns {string}
	 */
	_renderAdminDeviceOidTable(i18n, devices) {
		let html = `<table style="margin-top:6px">
...
`;
  ...
  <td><code style="font-size:11px;color:var(--text-faint)">${esc(dev.id)}</code></td>
	...
	/**
	 * Render scripts chapter for User profile — cards when desc exists; otherwise family summary (not empty).
	 *
	 * @param {object} docModel Document model
	 * @returns {string} HTML
	 */
	renderUserScriptsChapter(docModel) {
		...
		const i18n = this.i18n;
		const enabled = (scriptsData.scripts || []).filter(s => s.enabled);
		if (enabled.length === 0) {
			return '';
		}

		const withDesc = enabled.filter(s => s.desc && String(s.desc).trim());
		let html = `<h2 id="scripts">${esc(i18n.t('automations'))}</h2>\n`;

		if (withDesc.length > 0) {
			html += `<div class="adapter-list">\n`;
			for (const script of withDesc) {
				const line = String(script.desc).split('\n')[0].trim();
				html += `<div class="adapter-card">
  <strong>${esc(script.name)}</strong>
  <p class="adapter-desc">${esc(line)}</p>
</div>\n`;
			}
			html += `</div>\n`;
			const omitted = enabled.length - withDesc.length;
			if (omitted > 0) {
				html += `<p style="font-size:13px;color:var(--text-faint);line-height:1.5;margin:12px 0">${esc(
					i18n.t('userAutomationsOmittedNoDesc', omitted),
				)}</p>\n`;
			}
		}

		if (withDesc.length === 0) {
			html += this._renderFamilyAutomationsSummary(docModel, 'user');
		} else if (enabled.length - withDesc.length > 0) {
			html += this._renderFamilyAutomationsTriggerMix(docModel);
		}
	...
	renderUserAdaptersChapter(docModel) {
		...
		let html = `<details class="autodoc-collapsible-section" id="adapter-instances" data-autodoc-default-closed="1">
<summary class="maint-details-summary">${esc(i18n.t('connectedSystems'))} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${active.length})</span></summary>
...
`;
		...
		if (active.length === 0) {
			html += `<p style="color:var(--text-faint);font-size:14px"><em>${esc(i18n.t('noAdaptersVisible'))}</em></p>\n`;
		} else {
			html += `<div class="adapter-card-grid">\n`;
			for (const adapter of active) {
				...
				const contextNote = manualNote ? `<div class="manual-context-box">${esc(manualNote)}</div>` : '';
				const cls = `adapter-card-item${manualNote ? ' table-row--manual-note' : ''}`;
				html += `<div class="${cls}">
  <div><strong>${esc(displayName)}</strong></div>
  ${adapter.desc ? `<div class="aci-desc">${esc(adapter.desc)}</div>` : ''}
  ${contextNote}
</div>\n`;
			}
			html += `</div>\n`;
...
function copyPageUrl(btn) {
  ...
  var ok = function() {
    if (btn) { btn.textContent = '✅ ${copiedLabel}'; window.setTimeout(function(){ btn.textContent = '📋 ${copyLabel}'; }, 2000); }
  };
...
`;
		...
		const mc = docModel.manualContext;
		const mermaidWelcome =
			mc && mc.mermaidDiagram && String(mc.mermaidDiagram).trim()
				? this._renderMermaidDiagramHtml(mc, docModel, PROFILE_ONBOARDING)
				: '';
		const mermaidAutoWelcome =
			mc && mc.autoHostTopologyMermaid && String(mc.autoHostTopologyMermaid).trim()
				? this._renderAutoTopologyMermaidHtml(mc, docModel, PROFILE_ONBOARDING)
				: '';
...
${descHtml}${mermaidWelcome}${mermaidAutoWelcome}${contactHtml}${setupHint}<div style="clear:both"></div>
<hr class="section-divider">
`;
	}
...
`;
	...
	renderOnboardingAutomations(docModel) {
		const scriptsData = docModel.scripts;
		const i18n = this.i18n;
		const showNames = onboardingGuestShowsScriptNames(this.adapter.config);
		...
		if (!showNames) {
			html += this._renderFamilyAutomationsSummary(docModel, 'onboarding');
			html += '<hr class="section-divider">\n';
			return html;
		}
		...
		const shown = ordered.slice(0, ONBOARDING_AUTOMATIONS_MAX);
		...
		if (shown.length > 0) {
			...
			for (const script of shown) {
				if (script.desc) {
					...
					html += `  <li style="color:var(--text-faint)"><strong>${esc(script.name)}</strong>${scheduleMark(script)}</li>\n`;
				}
			}
			html += `</ul>\n`;
		}
		...
		if (omitted > 0) {
			html += `<p style="margin-top:8px;font-size:13px;color:var(--text-faint);line-height:1.5">${esc(
				i18n.t('onboardingAutomationsOmitted', omitted),
			)}</p>\n`;
		}
	...
	/**
	 * One table row for automation overview (time-based entries only).
	 *
	 * @param {object} row Overview entry
	 * @param {object} i18n
	 * @param {boolean} [useCronHint] Human-readable CRON via describeCron
	 * @returns {string}
	 */
	_renderAutomationOverviewRow(row, i18n, useCronHint) {
		const whenRaw = row.detail || '—';
		const whenDisplay = useCronHint ? this.describeCron(whenRaw, i18n) : whenRaw || '—';
		const whenTitle =
			useCronHint && whenRaw !== whenDisplay && String(whenRaw).trim() ? ` title="${esc(String(whenRaw))}"` : '';
		...
		const anchor = row.anchor ? `#${row.anchor}` : '';
		return `<tr>
  <td><strong>${esc(row.name)}</strong><br><small style="color:var(--text-faint);word-break:break-all"><a href="${esc(anchor)}">${esc(row.id)}</a></small></td>
  <td><small${whenTitle}>${esc(whenDisplay)}</small></td>
  <td>${statusBadge}</td>
</tr>\n`;
	}

	/**
	 * Script trigger mix + rule-engine instances AutoDoc does not parse.
	 *
	 * @param {object} overview `docModel.automationOverview`
	 * @returns {string} HTML fragment or empty
	 */
	_renderAutomationTransparencyBlock(overview) {
		const i18n = this.i18n;
		const stats = overview.scriptStats;
		const engines = overview.ruleEngines || [];
		if (!stats || (!stats.enabledTotal && engines.length === 0)) {
			return '';
		}
		let html = `<div class="manual-context-box" style="margin:16px 0;padding:12px 14px;font-size:13px;line-height:1.5">
<h3 style="margin:0 0 8px;font-size:14px">${esc(i18n.t('automationTransparencyTitle'))}</h3>`;
		if (stats.enabledTotal > 0) {
			const breakdown = formatScriptTriggerBreakdown(stats, (key, ...args) => i18n.t(key, ...args));
			html += `<p style="margin:0 0 8px">${esc(i18n.t('automationTransparencyScriptsIntro', stats.enabledTotal))}`;
			if (breakdown) {
				html += ` ${esc(breakdown)}.`;
			}
			html += ` <a href="#scripts">${esc(i18n.t('automationTransparencyScriptsLink'))}</a></p>`;
		}
		if (engines.length > 0) {
			html += `<p style="margin:8px 0 0;font-size:12px;color:var(--text-muted)">${esc(i18n.t('automationTransparencyRuleEnginesIntro'))}</p>
<ul style="margin:4px 0 0;padding-left:18px">`;
			for (const e of engines) {
				html += `<li>${esc(i18n.t('automationRuleEngineRow', e.title, e.instanceCount))}</li>`;
			}
			html += `</ul>`;
		}
		html += `<p style="margin:10px 0 0;font-size:12px;color:var(--text-faint)">${esc(i18n.t('automationTransparencyFootnote'))}</p>
</div>`;
		return html;
	}

	/**
	 * Admin: time-based automation overview (CRON / schedule mode only — not event scripts).
	 *
	 * @param {object} docModel Document model
	 * @returns {string} HTML
	 */
	renderAutomationOverviewChapter(docModel) {
		const overview = docModel.automationOverview;
		...
		const scriptStats = overview.scriptStats || {};

		const sectionDefs = [
			{ kind: 'script-cron', titleKey: 'automationOverviewSectionScriptCron', cron: true },
			{ kind: 'schedule-object', titleKey: 'automationOverviewSectionScheduleObject', cron: false },
			{ kind: 'adapter-schedule', titleKey: 'automationOverviewSectionAdapterSchedule', cron: true },
			{ kind: 'adapter-restart', titleKey: 'automationOverviewSectionAdapterRestart', cron: true },
		];

		let html = `<h2 id="automation-overview">${esc(i18n.t('automationOverviewTitle'))}</h2>
<p style="font-size:13px;color:var(--text-faint);line-height:1.5;margin-bottom:12px">${esc(i18n.t('automationOverviewIntro'))}</p>
<div class="stat-grid">
  <div class="stat-card"><div class="num">${esc(counts.total)}</div><div class="label">${esc(i18n.t('automationOverviewStatTotal'))}</div></div>
  <div class="stat-card"><div class="num">${esc(counts.enabled)}</div><div class="label">${esc(i18n.t('automationOverviewStatActive'))}</div></div>
`;
		if (scriptStats.enabledTotal > 0) {
			html += `  <div class="stat-card"><div class="num">${esc(scriptStats.enabledTotal)}</div><div class="label">${esc(i18n.t('enabledScripts'))}</div></div>\n`;
		}
		html += `</div>\n`;
		html += this._renderAutomationTransparencyBlock(overview);

		const tableHead = `<table class="automation-overview-table">
<thead><tr>
  <th>${esc(i18n.t('automationOverviewSource'))}</th>
  <th>${esc(i18n.t('automationOverviewWhen'))}</th>
  <th>${esc(i18n.t('automationOverviewStatus'))}</th>
</tr></thead>
<tbody>
`;

		for (const def of sectionDefs) {
			const rows = sections[def.kind] || [];
			if (rows.length === 0) {
				continue;
			}
			const active = rows.filter(r => r.enabled);
			const inactive = rows.filter(r => !r.enabled);
			html += `<h3 style="margin-top:20px;font-size:16px">${esc(i18n.t(def.titleKey))} <span style="font-weight:400;color:var(--text-faint)">(${rows.length})</span></h3>\n`;
			if (active.length > 0) {
				html += tableHead;
				for (const row of active) {
					html += this._renderAutomationOverviewRow(row, i18n, def.cron);
				}
				html += `</tbody>\n</table>\n`;
			}
			if (inactive.length > 0) {
				const inactiveLabel = i18n.t('automationOverviewDisabledGroup').replace('{0}', inactive.length);
				html += `<details style="margin-top:8px;margin-bottom:12px">
<summary class="maint-details-summary script-disabled-summary">${esc(inactiveLabel)}</summary>
${tableHead}`;
		...
		html += `<p style="font-size:12px;color:var(--text-faint);margin-top:12px;border-left:3px solid var(--border);padding-left:10px">${esc(i18n.t('automationOverviewLimits'))}</p>
<hr class="section-divider">\n`;
		return html;
	}
...
`;
	...
	renderOnboardingAdapters(docModel) {
		...
		let html = `<details class="autodoc-collapsible-section" style="margin-bottom:12px" id="adapter-instances" data-autodoc-default-closed="1">
...
`;
  ...
  ${note ? `<div class="manual-context-box">${esc(note)}</div>` : ''}
	...
	/**
	 * User-facing device card with optional adapter source hint.
	 *
	 * @param {object} dev Room device row from document model
	 * @returns {string} HTML
	 */
	_renderDeviceCard(dev) {
		const liveHtml = this._renderLiveValue(dev);
		const adapterName = dev.adapterName ? String(dev.adapterName) : '';
		const deviceName = dev.deviceName ? String(dev.deviceName) : '';
		const showSource = adapterName && !deviceName.toLowerCase().includes(adapterName.toLowerCase());
		const sourceHtml = showSource
			? ` <span class="device-source" style="font-size:11px;color:var(--text-faint)">${esc(this.i18n.t('deviceSourceHint', adapterName))}</span>`
			: '';
		return `<div class="device-card">
  <span class="device-icon">${esc(dev.icon || '📦')}</span>
  <span class="device-name">${esc(dev.deviceName)}</span>${sourceHtml}${liveHtml}
</div>\n`;
	}
	 ...
	 * @param {'admin'|'user'|'onboarding'|null} [omitLinkForProfile] No self-link for the current profile; for
	 *  **onboarding (guest)**, also no link to the **user/family** document.
	 * @returns {string}
	 */
...
<ol style="margin:0;padding-left:20px">
  ...
  <li>${esc(i18n.t('troubleshootSnapshotNodeStep3'))}</li>
	...
	_renderGuestHelpChapter(docModel, profile) {
		...
		if (!guestHelpChapterHasContent(mc, docModel, profile)) {
			return '';
		}
		const i18n = this.i18n;
		...
		if (!(profile === PROFILE_USER && docModel && docModel._autodocUserPublicLinksInManual)) {
...
<hr class="section-divider">\n`;
	...
	/**
	 * Owner “playbook”: order of operations, must-dos, do-not rules (household wording).
	 *
	 * @param {object} [manualContext]
	 * @returns {string}
	 */
	_renderOwnerPlaybookChapter(manualContext) {
		const note = manualContext && manualContext.ownerPlaybookNote;
		if (!note || !String(note).trim()) {
			return '';
		}
		const i18n = this.i18n;
		return `<h2 id="owner-playbook">${esc(i18n.t('ownerPlaybookTitle'))}</h2>
<p style="font-size:14px;color:var(--text-faint);margin:0 0 10px">${esc(i18n.t('ownerPlaybookIntro'))}</p>
<div class="manual-context-box" style="font-size:15px;line-height:1.7">${formatMultilineManualHtml(note)}</div>
<hr class="section-divider">\n`;
	}
...
${sub.join('\n')}
	...
	renderNav(profile, docModel) {
		...
		if (profile === PROFILE_ONBOARDING) {
			...
			if (!oh('guestHelp') && mcOb && guestHelpChapterHasContent(mcOb, dm, PROFILE_ONBOARDING)) {
			...
			if (!oh('mermaid') && mcOb && mcOb.mermaidDiagram && String(mcOb.mermaidDiagram).trim()) {
				obLines.push(`<li><a href="#mermaid-diagram">${esc(i18n.t('mermaidDiagramTitle'))}</a></li>`);
			}
			// auto-topology never shown in onboarding — nav entry omitted
			if (!oh('rooms')) {
			...
			if (!oh('ownerPlaybook') && mcOb && mcOb.ownerPlaybookNote && String(mcOb.ownerPlaybookNote).trim()) {
				obLines.push(`<li><a href="#owner-playbook">${esc(i18n.t('ownerPlaybookTitle'))}</a></li>`);
			}
			if (!oh('automations')) {
			...
			if (
				mcUs &&
				((!uh('manual') && (mcUs.description || mcUs.contact || mcUs.notes)) ||
					(!uh('mermaid') && mcUs.mermaidDiagram && String(mcUs.mermaidDiagram).trim()) ||
					(!uh('mermaidAuto') && mcUs.autoHostTopologyMermaid && String(mcUs.autoHostTopologyMermaid).trim()))
			) {
			...
			if (!uh('guestHelp') && mcUs && guestHelpChapterHasContent(mcUs, dm, PROFILE_USER)) {
			...
			if (!uh('ownerPlaybook') && mcUs && mcUs.ownerPlaybookNote && String(mcUs.ownerPlaybookNote).trim()) {
				usLines.push(`<li><a href="#owner-playbook">${esc(i18n.t('ownerPlaybookTitle'))}</a></li>`);
			}
			if (!uh('adapters')) {
...
`;
	...
	_formatQuickStartSystemLine(item, forOnboardingGuest) {
		...
		// Guest + privacy: do not show script rows at all — `common.desc` is no reliable guest-facing
		// “description” (schema: optional group-purpose text; often empty or technical).
		...
		switch (item.kind) {
			...
			case 'automationCount':
				return i18n.t('qsAutomationCount', item.n);
			case 'function':
	...
	_renderRoomGuideCardHtml(rg) {
		...
		const hi = (rg.highlights || [])
			...
			.join('');
		const totalDevices = rg.deviceCount != null ? rg.deviceCount : (rg.highlights || []).length;
		const hiddenCount = totalDevices - (rg.highlights || []).length;
		const moreHint =
			hiddenCount > 0
				? `<li style="color:var(--text-muted);font-size:13px;list-style:none;margin-left:-18px;margin-top:4px">${esc(
						i18n.t('qsRoomCardMore', hiddenCount),
					)}</li>`
				: '';
  ...
  <div style="font-weight:600;margin-bottom:8px">${esc(rg.name)} <span style="font-size:12px;color:var(--text-faint)">(${esc(
		i18n.t('qsRoomCardDevices', totalDevices),
  )})</span></div>
  ...
  ${moreHint}
	...
	renderUserAtAGlance(docModel) {
		...
		const roomsCross = showRoomsLink
			? `<p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">${esc(i18n.t('qsSeeFullRoomsBefore'))}<a href="#rooms-and-functions">${esc(i18n.t('roomsAndFunctions'))}</a>${esc(i18n.t('qsSeeFullRoomsAfter'))}</p>\n`
			: '';
...
${roomsCross}${sysLines ? `<h3>${esc(i18n.t('qsSystemTitle'))}</h3>\n<ul class="content-list">\n${sysLines}\n</ul>\n` : ''}
${
	roomBlock
...
`;
	...
	renderQuickStart(docModel) {
		...
		if (qs.hasContent) {
			...
			const roomsCross =
				(qs.roomGuides || []).length > 0 && !this.onboardingChapterHidden(docModel, 'rooms')
					? `<p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">${esc(i18n.t('qsSeeFullRoomsBefore'))}<a href="#rooms">${esc(i18n.t('yourRooms'))}</a>${esc(i18n.t('qsSeeFullRoomsAfter'))}</p>`
					: '';
...
`
				: '';
			structured = `${head}${roomsCross}${sysBlock}${roomPart}`;
		}

		const enabledScripts =
			docModel.scripts && docModel.scripts.enabledScripts != null ? docModel.scripts.enabledScripts : 0;
		const scriptStatCard =
			enabledScripts > 0
				? `  <div class="stat-card"><div class="num">${esc(enabledScripts)}</div><div class="label">${esc(i18n.t('enabledScripts'))}</div></div>\n`
				: '';
...
<ul class="content-list">
  <li>${esc(i18n.t('nextStepsOnboarding1'))}</li>
  <li>${esc(i18n.t('nextStepsOnboarding2'))}</li>
  <li>${esc(i18n.t('nextStepsOnboarding3'))}</li>
</ul>
<hr class="section-divider">
`;
	}
...
`;
		...
		if (
			...
			docModel.docChangeSinceLastRun &&
			this.adapter.config.hideAdminDeltaSinceLastRun !== true
		) {
			const block = buildDocChangeSinceLastRun(docModel.docChangeSinceLastRun, i18n);
			if (!block.skip) {
				html += `<div class="note-box" id="doc-change-since-last-run"><strong>${esc(i18n.t('docChangeSinceLastTitle'))}</strong>
<p style="margin:8px 0 6px;font-size:13px;color:var(--text-muted)">${esc(i18n.t('docChangeSinceLastNote'))}</p>
<p style="margin:0 0 ${block.lines.length ? '8px' : '0'};font-size:14px">${esc(block.headline)}</p>`;
				if (block.lines.length > 0) {
					html += `<ul style="margin:0;padding-left:20px;font-size:13px;color:var(--text)">\n`;
					for (const row of block.lines) {
						html += `  <li><span style="color:var(--link);font-weight:600">${esc(row.typeLabel)}</span> ${esc(row.detail)}</li>\n`;
					}
					html += `</ul>\n`;
				}
				html += `</div>\n`;
		...
		html += `<h3>${esc(i18n.t('projectInformation'))}</h3>
<dl class="meta">
...
`;
		...
		if (profile === PROFILE_ADMIN) {
			html += `<p id="doc-transparency-limits" style="font-size:12px;line-height:1.45;color:var(--text-muted);margin:14px 0 0;padding-top:10px;border-top:1px solid var(--border)">${esc(i18n.t('docTransparencyLimitsShort'))}</p>
`;
			const ao = docModel.automationOverview;
			const breakdown =
				ao && ao.scriptStats && ao.scriptStats.enabledTotal
					? formatScriptTriggerBreakdown(ao.scriptStats, (key, ...args) => i18n.t(key, ...args))
					: '';
			if (breakdown) {
				html += `<p style="font-size:12px;line-height:1.45;color:var(--text-muted);margin:8px 0 0">${esc(i18n.t('docTransparencyScriptStats', ao.scriptStats.enabledTotal))} ${esc(breakdown)}.</p>
`;
			}
		}
		html += '<hr class="section-divider">\n';
...
`;
		...
		if (profile === PROFILE_ADMIN) {
			...
			// Admin: card grid — enabled first, disabled collapsed; whole section starts collapsed
			...
			/**
			 * Build a card element for an adapter.
			 *
			 * @param {object} adapter Adapter object from document model
			 * @returns {string} HTML card div
			 */
			const buildAdapterRow = adapter => {
				const hasTitle = adapter.title && adapter.title !== adapter.name;
				const displayTitle = hasTitle ? adapter.title : adapter.name;

				let instanceDetailsHtml = '';
				if (!config.hideInstanceDetailsInMarkdown) {
					const instRows = adapter.instances
						.map(inst => {
							let line = `<div class="aci-inst-row"><span>${esc(inst.id)}</span><span class="badge ${inst.enabled ? 'badge-ok' : 'badge-off'}">${esc(inst.enabled ? i18n.t('enabled') : i18n.t('disabled'))}</span><span style="color:var(--text-faint)">v${esc(inst.version || '?')}</span>`;
						...
						.join('');
					if (instRows) {
						instanceDetailsHtml = `<details class="aci-instances"><summary>${esc(i18n.t('instanceDetails'))}</summary><div class="aci-instances-content">${instRows}</div></details>`;
					}
				}
				...
				if (ct && ct !== 'none' && ct !== '') {
					...
					metaBadges.push(`<span class="badge badge-meta">${ctLabel}</span>`);
				}
				...
				if (ds && ds !== 'none' && ds !== '' && ds !== 'assumption') {
					...
					metaBadges.push(`<span class="badge badge-meta">${dsLabel}</span>`);
				}
				if (adapter.tier) {
					const tierLabel =
						adapter.tier === 1
					...
					metaBadges.push(`<span class="badge badge-meta">${tierLabel}</span>`);
				}
				...
				const manualNoteText =
					...
					docModel.manualContext.adapters[adapter.name]
						? docModel.manualContext.adapters[adapter.name]
						: null;
				const hasAdapterManual = !!manualNoteText;
				const manualNoteHtml = manualNoteText
					? `<div class="aci-manual-note">${esc(manualNoteText)}</div>`
					: '';

				const countsHtml = `<span class="aci-counts">${esc(adapter.totalInstances)}\u00a0/\u00a0${esc(adapter.enabledInstances)}\u00a0${esc(i18n.t('enabledShort'))}</span>`;

				return `<div class="adapter-card-item${hasAdapterManual ? ' table-row--manual-note' : ''}">
  <div class="aci-header"><strong title="${esc(displayTitle)}">${esc(displayTitle)}</strong>${hasTitle ? `<small>${esc(adapter.name)}</small>` : ''}</div>
  ${adapter.desc ? `<div class="aci-desc">${esc(adapter.desc)}</div>` : ''}
  <div class="aci-meta">${metaBadges.join('')}${countsHtml}</div>
  ${manualNoteHtml}${instanceDetailsHtml}
</div>\n`;
			};

			// Summary line: "Adapter-Details (14 aktiv, 3 deaktiviert)"
			const sectionCountParts = [`${enabledAdapters.length}\u00a0${i18n.t('enabledShort')}`];
			if (disabledAdapters.length > 0) {
				sectionCountParts.push(`${disabledAdapters.length}\u00a0${i18n.t('disabled')}`);
			}
			const sectionCountHtml = `<span class="adapter-section-counts">(${sectionCountParts.join(', ')})</span>`;

			html += `<details id="adapter-section">
<summary class="adapter-section-summary">${esc(i18n.t('adapterDetails'))}${sectionCountHtml}</summary>
...
<div class="adapter-card-grid" id="adapter-enabled-body">
`;
...
<div class="adapter-card-grid" id="adapter-disabled-body" style="margin-top:8px">
`;
...
`;
						if (roomNote) {
							html += `<div class="manual-context-box" style="margin-top:6px">${esc(roomNote)}</div>\n`;
						}
						html += this._renderAdminDeviceOidTable(i18n, devices);
						html += `</details>\n`;
					}
					html += `</div>\n</details>\n`;
				}
			...
			if (profile === PROFILE_ADMIN && roomsData.functions.length > 0) {
				const totalFnDevices = roomsData.functions.reduce(
					(s, fn) => s + (fn.deviceCount != null ? fn.deviceCount : (fn.devices || []).length),
					0,
				);
				html += `<h3 style="margin-top:24px">${esc(i18n.t('functions'))}</h3>
<details class="autodoc-collapsible-section" data-autodoc-default-closed="1" style="margin-bottom:12px">
<summary class="maint-details-summary">${esc(i18n.t('showFunctions'))} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${roomsData.functions.length} ${esc(i18n.t('functions'))}, ${totalFnDevices} ${esc(i18n.t('members'))})</span></summary>
<div style="margin-top:8px">
`;
				for (const fn of roomsData.functions) {
					const devices = fn.devices || [];
					const devCount = fn.deviceCount != null ? fn.deviceCount : devices.length;
					html += `<details style="margin-bottom:6px;margin-left:8px">
<summary class="maint-details-summary">${esc(fn.name)} <span style="font-weight:400;color:var(--text-faint);font-size:13px">(${devCount} ${esc(i18n.t('members'))})</span></summary>
`;
					if (devCount === 0) {
						html += `<p style="color:var(--text-faint);font-size:14px;margin-top:6px"><em>${esc(i18n.t('noDevicesInFunction'))}</em></p>\n`;
					} else {
						html += this._renderAdminDeviceOidTable(i18n, devices);
					}
					html += `</details>\n`;
...
`;
	...
	/**
	 * Owner-written Mermaid diagram (Phase 5.x.3). Rendered client-side in HTML via script in {@link HtmlRenderer#wrapPage}.
	 *
	 * @param {object} manualContext
	 * @param {object|null} docModel
	 * @param {string} profile PROFILE_ADMIN | PROFILE_USER | PROFILE_ONBOARDING
	 * @returns {string}
	 */
	_renderMermaidDiagramHtml(manualContext, docModel, profile) {
		const raw = manualContext && manualContext.mermaidDiagram && String(manualContext.mermaidDiagram).trim();
		if (!raw) {
			return '';
		}
		if (profile === PROFILE_USER && docModel && this.userChapterHidden(docModel, 'mermaid')) {
			return '';
		}
		if (profile === PROFILE_ONBOARDING && docModel && this.onboardingChapterHidden(docModel, 'mermaid')) {
			return '';
		}
		const i18n = this.i18n;
		const diagramBody = `<p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">${esc(i18n.t('mermaidDiagramIntro'))}</p>
<div class="mermaid-wrap">
<pre class="mermaid">${esc(raw)}</pre>
</div>`;
		return `<details class="autodoc-collapsible-section" id="mermaid-diagram" data-autodoc-default-closed="1">
<summary class="maint-details-summary">${esc(i18n.t('mermaidDiagramTitle'))}</summary>
<div style="margin-top:8px">${diagramBody}</div>
</details>\n`;
	}

	/**
	 * Auto-generated host/instance Mermaid (Phase 5.x.3). Same hide chapter as owner diagram (`mermaid`).
	 *
	 * @param {object} manualContext
	 * @param {object|null} docModel
	 * @param {string} profile PROFILE_ADMIN | PROFILE_USER | PROFILE_ONBOARDING
	 * @returns {string}
	 */
	_renderAutoTopologyMermaidHtml(manualContext, docModel, profile) {
		const raw =
			manualContext &&
		...
		// Auto-topology is never shown in Onboarding (too technical for guests)
		if (profile === PROFILE_ONBOARDING) {
			return '';
		}
		if (profile === PROFILE_USER && docModel && this.userChapterHidden(docModel, 'mermaidAuto')) {
			return '';
		}
		if (profile === PROFILE_ADMIN && docModel && this.adminChapterHidden(docModel, 'mermaidAuto')) {
			return '';
		}
		const i18n = this.i18n;
		const diagramBody = `<p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">${esc(i18n.t('mermaidAutoTopologyIntro'))}</p>
<div class="mermaid-wrap">
<pre class="mermaid">${esc(raw)}</pre>
</div>`;
		if (profile === PROFILE_ADMIN || profile === PROFILE_USER) {
			return `<details class="autodoc-collapsible-section" id="mermaid-diagram-auto" data-autodoc-default-closed="1">
<summary class="maint-details-summary">${esc(i18n.t('mermaidAutoTopologyTitle'))}</summary>
<div style="margin-top:8px">${diagramBody}</div>
</details>\n`;
		}
		return `<h3 id="mermaid-diagram-auto">${esc(i18n.t('mermaidAutoTopologyTitle'))}</h3>
${diagramBody}\n`;
	}
	 ...
	 * @param {string} [profile] `admin`: guest/routines as h3 here; `user`: description/contact/notes plus public doc links (same as admin placement); guest routines/playbook stay separate h2 sections.
	 * @param {object|null} [docModel] Optional — hide flags for user mermaid/manual core
	 * @returns {string} Manual context HTML
	 */
	renderManualContext(manualContext, profile = PROFILE_ADMIN, docModel = null) {
		const i18n = this.i18n;
		const hideUserManualCore = profile === PROFILE_USER && docModel && this.userChapterHidden(docModel, 'manual');
		let html = `<h2 id="manual-information">${esc(i18n.t('manualInformation'))}</h2>\n`;

		if (manualContext.description && !hideUserManualCore) {
			html += `<h3>${esc(i18n.t('description'))}</h3>\n<p>${esc(manualContext.description)}</p>\n`;
		}
		if (manualContext.contact && !hideUserManualCore) {
			html += `<h3>${esc(i18n.t('contact'))}</h3>\n<p>${esc(manualContext.contact)}</p>\n`;
		}
		if (manualContext.notes && !hideUserManualCore) {
		...
		if (
			...
			publicLinksRelevantForProfile(manualContext && manualContext.troubleshootPublicLinks, PROFILE_USER)
		) {
			const pubLinks = this._renderTroubleshootPublicLinksHtml(manualContext, false, 'user');
			if (pubLinks) {
				html += pubLinks;
				docModel._autodocUserPublicLinksInManual = true;
			}
		}
		if (profile === PROFILE_ADMIN) {
			html += this._renderTroubleshootPublicLinksHtml(manualContext, true, 'admin');
			...
			if (manualContext.ownerPlaybookNote && String(manualContext.ownerPlaybookNote).trim()) {
				html += `<h3>${esc(i18n.t('ownerPlaybookTitle'))}</h3>\n<p style="font-size:13px;color:var(--text-faint)">${esc(i18n.t('ownerPlaybookIntro'))}</p>\n<div class="manual-context-box">${formatMultilineManualHtml(manualContext.ownerPlaybookNote)}</div>\n`;
			}
		}

		html += this._renderMermaidDiagramHtml(manualContext, docModel, profile);
		html += this._renderAutoTopologyMermaidHtml(manualContext, docModel, profile);
		html += '<hr class="section-divider">\n';
		return html;
	}
...
`;
	...
	renderMaintenanceChapter(docModel) {
		...
		const renderDimension = (checks, score, title, desc) => {
			...
			let s = `<h4 style="margin:18px 0 4px">${esc(title)} <span style="font-weight:normal;font-size:13px;color:var(--text-muted)">${esc(score)}%</span></h4>`;
			if (desc) {
				s += `<p style="font-size:12px;color:var(--text-faint);margin:0 0 6px">${esc(desc)}</p>`;
			}
			s += `<div class="score-bar" style="margin-bottom:8px"><div class="score-bar-fill" style="width:${esc(score)}%;background:${color}"></div></div>\n`;
			if (checks.length > 0) {
				s += `<ul class="checklist" style="margin-bottom:4px">\n`;
				for (const item of checks) {
					const icon = item.ok ? '✅' : '⚠️';
					const label = checkLabels[item.key] || item.key;
					const issueSuffix =
						!item.ok && typeof item.count === 'number' && item.count > 0
							? ` (${esc(String(item.count))})`
							: '';
					const badge = item.ok
						? `<span class="badge badge-ok">${esc(i18n.t('checkOk'))}</span>`
						: `<span class="badge badge-off">${esc(i18n.t('checkIssue'))}${issueSuffix}</span>`;
					s += `  <li>${icon} ${esc(label)} ${badge}</li>\n`;
				}
				s += `</ul>\n`;
			}
			return s;
		...
		const overallColor = m.score >= 80 ? '#28a745' : m.score >= 50 ? '#ffc107' : '#dc3545';
...
`;
		...
		html += `<p style="margin-top:16px"><strong>${esc(i18n.t('documentationScore'))}:</strong> ${esc(m.score)}%</p>
<div class="score-bar"><div class="score-bar-fill" style="width:${esc(m.score)}%;background:${overallColor}"></div></div>
`;
...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
	 ...
	 * Snapshot of what autodoc read for this documentation export, plus narrow automated rules (Node heuristic)
	 * and a separate general maintenance reminder.
	 *
	 * @param {object} docModel Document model
	...
	renderDiagnosis(docModel) {
		...
		if (isNodeVersionFlaggedForDiagnosis(nodeVer)) {
			nodeBullet = i18n.t('nodeVersionOutdated').replace('{0}', nodeVer || '—');
		} else if (nodeVer) {
			nodeBullet = i18n.t('diagNodeCheckOk').replace('{0}', nodeVer);
		} else {
			nodeBullet = i18n.t('diagNodeVersionMissing');
		}
		const autoChecksHtml = `  <li>${esc(nodeBullet)}</li>`;
...
<p style="font-size:13px;color:var(--text-muted);margin:0 0 1rem 0">${esc(i18n.t('diagnosisChapterIntro'))}</p>
...
function copyForumData(btn) {
  try {
    ...
    var ok = function() {
      ...
      window.setTimeout(function(){ btn.textContent = '📋 ${copyLabel}'; }, 2500);
    };
    if (navigator.clipboard && window.isSecureContext) {
...
<p style="font-size:13px;color:var(--text-muted);margin:0 0 0.75rem 0">${esc(i18n.t('diagAutomatedChecksIntro'))}</p>
<ul class="content-list">
${autoChecksHtml}
</ul>
<h3>${esc(i18n.t('diagMaintenanceReminders'))}</h3>
<p style="font-size:13px;color:var(--text-muted);margin:0 0 0.75rem 0">${esc(i18n.t('diagMaintenanceRemindersIntro'))}</p>
<ul class="content-list">
  <li>${esc(i18n.t('osUpdateHint'))}</li>
</ul>
<hr class="section-divider">
`;
	}
	...
	/**
	 * Admin HTML: chapter «Betrieb – Referenz» / Operational reference (after Diagnosis).
	 *
	 * Blocks (top to bottom):
	 * 1. **Adapter startet nicht** — always
	 * 2. **Adapter verbunden** issue — always (generic adapters)
	 * 3. **Skript läuft nicht** — only if `docModel.scripts.totalScripts > 0`
	 * 4. **Dokumentation startet nicht automatisch (alle Auslöser aus)** — only if
	 *    `!hasAutomaticDocGenerationTrigger(this.adapter.config)` (see helper above)
	 *
	 * A short disclaimer at the top states that blocks are **generic** (not auto-validated).
	 *
	 * @param {object} docModel
	 * @returns {string}
	 */
	renderTroubleshooting(docModel) {
		...
		const cfg = this.adapter.config || {};
		const showDocNotGeneratedTroubleshooting = !hasAutomaticDocGenerationTrigger(cfg);

		return `<h2 id="troubleshooting">${esc(i18n.t('troubleshooting'))}</h2>
<p style="font-size:13px;color:var(--text-muted);margin:0 0 1rem 0">${esc(i18n.t('troubleshootingGenericDisclaimer'))}</p>
...
<ol class="content-list">
  ...
  <li>${esc(i18n.t('tsScriptNotRunning3'))}</li>
		...
		: ''
			...
			showDocNotGeneratedTroubleshooting
...
`;
  ...
  <div style="font-size:13px;color:var(--text-muted);margin-bottom:4px">${esc(localizeCompareSummary(entry, i18n))}</div>`;

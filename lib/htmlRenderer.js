 ...
 */
const RENDERER_VERSION = '2026.04.11.4';
...
<!-- qrcodejs 1.0.0 = upstream library on cdnjs, not ioBroker.autodoc adapter version -->
...
`;
  ...
  <span class="device-name">${esc(dev.deviceName)}</span>${liveHtml}
	...
	renderOnboardingHint(docModel) {
		const i18n = this.i18n;
		const hasManual =
			docModel.manualContext && (docModel.manualContext.description || docModel.manualContext.notes);
...
`;
	...
	renderAiSection(ai, opts) {
		...
		const i18n = this.i18n;
		return `${debugLead}<div class="ai-box">
...
<div class="adapter-filter-bar">
  ...
  <span id="${filterId}-count" style="font-size:12px;color:var(--text-faint)">${totalItems} ${esc(i18n.t('totalSuffix') || 'gesamt')}</span>
</div>
...
(function(){
  ...
  inp.addEventListener('input', function(){
    ...
    if (cnt) cnt.textContent = (term ? visible + ' / ' : '') + ${totalItems} + ' ${esc(i18n.t('totalSuffix') || 'gesamt')}';
...
`;
		...
		if (m.scriptsWithoutDescription.length > 0) {
			html += `<p style="font-size:13px;color:var(--text-faint);margin-top:12px;margin-bottom:6px">${esc(i18n.t('scriptsWithoutDescOptionalCaption'))}</p>\n`;

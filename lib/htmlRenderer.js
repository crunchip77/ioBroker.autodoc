 ...
 */
const RENDERER_VERSION = '2026.09.22.1';
...
`;
  ...
  <span class="device-name">${esc(dev.deviceName)}</span>${liveHtml}
	...
	renderOnboardingAutomations(docModel) {
		...
		if (!showNames) {
			html += `<p style="font-size:14px;color:var(--text-faint);line-height:1.55;margin-bottom:12px">${esc(
				i18n.t('onboardingAutomationsSummaryBody', active.length),
			)}</p>\n`;
			html += '<hr class="section-divider">\n';
			return html;
		}

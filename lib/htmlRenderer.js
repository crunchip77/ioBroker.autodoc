 ...
 */
const RENDERER_VERSION = '2026.04.27.5';
...
class HtmlRenderer {
	...
	renderUserHtml(docModel) {
		...
		if (!this.userChapterHidden(docModel, 'guestHelp')) {
			body += this._renderGuestHelpChapter(docModel);
		}
...
`;
  ...
  ${note ? `<div class="manual-context-box">${esc(note)}</div>` : ''}
	 ...
	 * @returns {string}
	 */
	_renderTroubleshootPublicLinksHtml(manualContext, includeAdmin) {
...
<ol style="margin:0;padding-left:20px">
  ...
  <li>${esc(i18n.t('troubleshootSnapshotNodeStep3'))}</li>
	 ...
	 * @returns {string}
	 */
	_renderGuestHelpChapter(docModel) {
		...
		parts.push(this._renderTroubleshootPublicLinksHtml(mc, false));

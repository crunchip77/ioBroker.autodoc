...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  nav { width: 240px; flex-shrink: 0; background: var(--nav-bg); color: var(--nav-text); padding: 24px 16px; position: sticky; top: 0; height: 100vh; overflow-y: auto; }
  ...
  #dark-toggle { position: fixed; bottom: 16px; right: 16px; background: var(--surface); border: 1px solid var(--border); border-radius: 50%; width: 40px; height: 40px; cursor: pointer; font-size: 18px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.2); z-index: 999; transition: transform 0.2s; }
...
`;
  ...
  ${contextNote}
	...
	renderOnboardingWelcome(docModel) {
		...
		const contactHtml = contact
			? `<div style="display:inline-flex;align-items:center;gap:8px;background:#e8f4fd;border:1px solid #b8d8f0;border-radius:6px;padding:8px 14px;font-size:14px;margin-bottom:20px">
  ...
  <span><strong>${esc(i18n.t('contact'))}:</strong> ${esc(contact)}</span>
		...
		// QR code section — generated client-side via CDN library
		const qrHtml = `<div id="qr-section" style="float:right;margin:0 0 16px 24px;text-align:center">
  <div id="qrcode" style="display:inline-block;background:#fff;padding:8px;border-radius:6px;border:1px solid var(--border)"></div>
  <div style="font-size:11px;color:var(--text-faint);margin-top:4px">${esc(i18n.t('scanToShare') || 'Scan to share')}</div>
</div>
<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js" onerror="document.getElementById('qr-section').style.display='none'" defer><\/script>
<script>
window.addEventListener('load', function() {
  ...
  } catch(e) { document.getElementById('qr-section').style.display='none'; }
});
<\/script>
`;
		...
		if (scriptsData.totalScripts === 0) {
			...
			if (profile === PROFILE_ADMIN) {
				...
				const buildScriptRow = script => {
					...
					return `<tr>
  <td>${nameCell}</td>
  <td><span class="badge badge-ok">${esc(i18n.t('active'))}</span></td>
  <td><small>${esc(script.triggerType)}</small></td>
  <td><small>${esc(script.desc || '—')}</small></td>
</tr>\n`;
				};

				const buildInactiveScriptRow = script => {
					const folderLabel = this.scriptFolderLabel(script.folder, i18n);
					const nameCell = `${esc(script.name)}<br><small style="color:#888">${esc(folderLabel)}</small>`;
					return `<tr>
  <td>${nameCell}</td>
  <td><span class="badge badge-off">${esc(i18n.t('inactive'))}</span></td>
  <td><small>${esc(script.triggerType)}</small></td>
  <td><small>${esc(script.desc || '—')}</small></td>
</tr>\n`;
				};
...
`;
					for (const script of inactiveScripts) {
						html += buildInactiveScriptRow(script);
					}

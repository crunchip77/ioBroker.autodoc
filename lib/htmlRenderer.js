...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
	...
	renderDiagnosis(docModel) {
		...
		const scriptNoDesc = (docModel.maintenance && docModel.maintenance.scriptsWithoutDescription) || [];
		if (scriptNoDesc.length > 0) {
			findings.push(i18n.t('diagFindingScripts', scriptNoDesc.length));
		}

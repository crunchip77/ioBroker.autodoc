...
<div class="search-wrap">
  <input type="search" id="doc-search" placeholder="Filter..." autocomplete="off" aria-label="Filter documentation">
...
`;
  ...
  <td>${esc(adapter.enabledInstances)}</td>
			...
			html += `</tbody>\n</table>\n<p class="no-results">No adapters match your filter.</p>\n`;
		} else if (profile === PROFILE_USER) {
...
`;
  ...
  ${profile === PROFILE_ADMIN ? `<td>${membersCell || '—'}</td>` : ''}
			...
			html += `</tbody>\n</table>\n<p class="no-results">No rooms match your filter.</p>\n`;
...
<p class="no-results">No functions match your filter.</p>
`;
			}
		}
...
`;
  ...
  <td><small>${esc(script.desc || '—')}</small></td>
			...
			html += `</tbody>\n</table>\n<p class="no-results">No scripts match your filter.</p>\n`;
			...
			if (profile === PROFILE_ADMIN) {
				...
				if (scriptsWithRefs.length > 0) {
					html += `<h3 id="state-references">State References</h3>
<p style="font-size:13px;color:#666;margin-bottom:12px">States referenced by scripts (detected via static analysis).</p>
<table>
<thead><tr><th>Script</th><th>Referenced States</th></tr></thead>
<tbody>
`;
				...
				if (sharedStates.length > 0) {
					html += `<h3>Shared States</h3>
<p style="font-size:13px;color:#666;margin-bottom:12px">States used by more than one script.</p>
<table>
<thead><tr><th>State ID</th><th>Used by Scripts</th></tr></thead>
<tbody>
`;
					...
					html += `</tbody>\n</table>\n<p class="no-results">No shared states match your filter.</p>\n`;
				}
			}
		}

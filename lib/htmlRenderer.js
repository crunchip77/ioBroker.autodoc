...
class HtmlRenderer {
	...
	renderAdminHtml(docModel) {
		...
		body += this.renderTroubleshooting();
...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
	...
	/**
	 * Render troubleshooting section (removed — content moved to appendices).
	 *
...
<dl class="meta">
  ...
  <dt>${esc(i18n.t('instancesDetected'))}</dt><dd>${esc(system.statistics.instanceCount)}</dd>
  <dt>${esc(i18n.t('stateObjectsScanned'))}</dt><dd>${esc(appendices.stateSummary.total)}</dd>
  <dt>${esc(i18n.t('platform'))}</dt><dd>${esc(system.primaryHost.platform)}</dd>

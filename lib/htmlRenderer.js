...
`;
		...
		if (m.checklist.length === 0) {
			html += `<p style="font-size:13px;color:var(--text-muted);margin:10px 0 12px"><em>${esc(i18n.t('maintenanceChecklistDisabled'))}</em></p>\n`;
		}
		if (m.checklist.length > 0) {
			...
			for (const item of m.checklist) {
				...
				const issueSuffix =
					!item.ok && typeof item.count === 'number' && item.count > 0 ? ` (${esc(String(item.count))})` : '';
				const badge = item.ok
...
${m.disabledInstances.map(inst => `<tr><td><small>${esc(inst.id)}</small></td><td>${esc(inst.title && inst.title !== inst.name ? inst.title : inst.name)}</td></tr>`).join('\n')}
		...
		if (m.checklist.length > 0 && m.checklist.every(item => item.ok)) {

 ...
 */
const RENDERER_VERSION = '2026.04.17.2';
...
<div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px">\n`;
  ...
  <span><strong>${esc(i18n.t('contact'))}:</strong> ${esc(contact)}</span>
		...
		const onboardingPublicUrl =
			(renderOptions && renderOptions.urls && renderOptions.urls.onboarding) || '';
	...
	renderHostDistribution(docModel) {
		...
		for (const hostName of Object.keys(instancesByHost).sort()) {
			...
			const instanceBadges = instances
				...
				.map(i => `<span style="display:inline-block;background:var(--th-bg);border:1px solid var(--border);border-radius:4px;padding:1px 6px;font-size:11px;margin:2px 2px 0 0;font-family:monospace">${esc(i.id)}</span>`)
				.join('');

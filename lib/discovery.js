...
class Discovery {
	 ...
	 * Walks the ioBroker object hierarchy (state → channel → device) for clearer names.
	 *
	 * @param {Array} rooms Array of room objects from readRooms()
	 * @returns {Promise<object>} Map of memberId → enriched device info
	 */
	async resolveRoomDevices(rooms) {
		...
		for (const memberId of allMembers) {
			const chain = [];
			for (const id of ancestorIds(memberId)) {
				...
				chain.push({
					id,
					type: obj.type || 'state',
					name: this.resolveI18nString(obj.common.name, lang) || id.split('.').pop(),
					role: obj.common.role || '',
					unit: obj.common.unit || '',
				});
			}
			...
			const best = chooseBestObject(chain);
			if (!best) {
				continue;
			}
			const canonicalId = resolveCanonicalDeviceKey(best, memberId);
			const sourceAdapter = parseAdapterInstance(memberId);
			const roleFromChain = chain.find(c => c.role && c.role.trim());
			result[memberId] = {
				deviceId: canonicalId,
				memberId,
				deviceName: best.name,
				role: best.role || (roleFromChain && roleFromChain.role) || '',
				type: best.type || '',
				unit: best.unit || '',
				sourceAdapter,
				adapterName: sourceAdapter ? sourceAdapter.split('.')[0] : '',
			};
		}
		return result;
	}

...
class Discovery {
	...
	async readHostResources(hosts) {
		...
		for (const host of hosts) {
			...
			// (js-controller already divides by 1048576 before storing the state)
			...
			// Process-level fallback — memRss / memHeapUsed are also in MB
			const rawProcMb = (memRss && memRss.val > 0) ? memRss.val
				: (memHeapUsed && memHeapUsed.val > 0 ? memHeapUsed.val : null);
			// Safety: if val looks like bytes (> 100 000), convert; otherwise use directly

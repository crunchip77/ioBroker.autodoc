...
class Discovery {
	...
	async readSystemConfig() {
		try {
			...
			return {
				...
				tempUnit: obj.common.tempUnit || '°C',
	...
	async readHostResources(hosts) {
		...
		for (const host of hosts) {
			...
			try {
				const [totalMem, freeMem, cpu, uptime] = await Promise.all([
					this.adapter.getForeignStateAsync(`system.host.${hostId}.memHeapTotal`).catch(() => null),
				...
				// Try system RAM if heap not useful — some hosts expose system RAM separately
				const [sysTotal, sysFree] = await Promise.all([
					this.adapter.getForeignStateAsync(`system.host.${hostId}.memRss`).catch(() => null),
					this.adapter.getForeignStateAsync(`system.host.${hostId}.memAvailable`).catch(() => null),
				]);
				result[host.name] = {
					totalMem: sysTotal && sysTotal.val ? sysTotal.val : (totalMem && totalMem.val ? totalMem.val : null),
					freeMem: sysFree && sysFree.val ? sysFree.val : (freeMem && freeMem.val ? freeMem.val : null),
					cpu: cpu && cpu.val !== null && cpu.val !== undefined ? cpu.val : null,
	...
	async collectRawData() {
		...
		const [pendingUpdates, lastBackup, hostResources, userData] = await Promise.all([

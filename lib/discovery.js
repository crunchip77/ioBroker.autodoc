...
/** Blind/shutter roles (Shelly Cover.Position etc.); not always exactly `level.blind`. */
const LIVE_STATE_BLIND_RE = /^level\.blind|^blind\./;
...
function isReadableLiveRole(role) {
	...
	return LIVE_STATE_BLIND_RE.test(role);
}
...
function liveRoleSortIndex(role) {
	...
	if (LIVE_STATE_BLIND_RE.test(role)) {
		return LIVE_STATE_ROLE_PRIORITY.indexOf('level.blind');
	}
	return 999;
}

	...
	async readHostResources(hosts) {
		...
		for (const host of hosts) {
			...
			try {
				...
				// freemem/totalmem are already in MB in ioBroker JS-controller
				// memRss / memHeapUsed are in bytes — use as process-level fallback
				const sysFreeMb = freemem && freemem.val > 0 ? freemem.val : null;
				const sysTotalMb = totalmem && totalmem.val > 0 ? totalmem.val : null;
				const procMb = memRss && memRss.val > 0
					? Math.round(memRss.val / 1024 / 1024)
					: (memHeapUsed && memHeapUsed.val > 0 ? Math.round(memHeapUsed.val / 1024 / 1024) : null);

				result[host.name] = {
					// system RAM in MB (native installs); null in Docker
					sysFreeMb,
					sysTotalMb,
					// ioBroker process RAM in MB (always available)
					procMb,

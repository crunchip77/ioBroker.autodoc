 ...
 *
 * @param {object|null|undefined} docModel
 * @returns {boolean}
 */
function userManualHtmlChapterWillRender(docModel) {
	...
	const hasCore =
		!hidden('manual') && !!(trim(mc.description) || trim(mc.contact) || trim(mc.notes));
...
function guestHelpChapterHasContent(manualContext, docModel, exportProfile) {
	...
	if (publicLinksRelevantForProfile(mc.troubleshootPublicLinks, exportProfile)) {
		if (
			exportProfile === PROFILE_USER &&
			docModel &&
			userManualHtmlChapterWillRender(docModel)
		) {

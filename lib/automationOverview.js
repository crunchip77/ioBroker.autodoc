...
const { buildScriptAutomationStats, findRuleEngineInstances } = require('./scriptAutomationStats');
 ...
 *
 * @param {object} params Inputs from the document model (scripts, schedule objects, adapters).
 ...
 * @returns {object} Grouped overview rows, script/rule-engine transparency stats, summary counts
 */
function buildAutomationOverview({ scripts, scheduleObjects, adapters } = {}) {
	const entries = [];
	const scriptList = (scripts && scripts.scripts) || [];
	const scriptStats = buildScriptAutomationStats(scriptList);
	const ruleEngines = findRuleEngineInstances(adapters);
	...
	const hasScriptContext = scriptStats.enabledTotal > 0 || ruleEngines.length > 0;
	...
	return {
		...
		scriptStats,
		ruleEngines,
		isEmpty: !hasScheduled && !hasScriptContext,
		eventScriptCount: Math.max(0, scriptStats.enabledTotal - counts.scriptCron),
	};
}

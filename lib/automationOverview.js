 ...
 * @returns {{ entries: Array<object>, sections: object, counts: object, isEmpty: boolean, eventScriptCount: number }} Grouped overview rows and summary counts
 */
function buildAutomationOverview({ scripts, scheduleObjects, adapters } = {}) {
	...
	const eventScriptCount = scriptList.filter(s => !(s.schedule && String(s.schedule).trim()) && s.enabled).length;

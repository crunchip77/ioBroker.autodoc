...
async function embedMermaidDiagramsInHtml(html, options = {}) {
	...
	if (mmdcFailureCount > 0) {
		...
		if (/libnss3|cannot open shared object|Failed to launch the browser process/i.test(firstMmdcError)) {
			hint =
				'Missing Chromium OS libraries (often libnss3 on Linux). Run scripts/install-chromium-deps-linux.sh on the host, or on buanet Docker set PACKAGES (see script header). Offline SVG needs mmdc — https://pptr.dev/troubleshooting';
		} else if (/sandbox|no-sandbox/i.test(firstMmdcError)) {
			hint =

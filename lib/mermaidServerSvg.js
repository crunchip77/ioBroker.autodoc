...
/**
 * Mermaid JSON for `mmdc -c` — subgraph cluster colors match autodoc HTML (see `htmlRenderer.js`).
 * Uses `theme: "base"`: Mermaid only applies `themeVariables` overrides on the **base** theme; with
 * `default` / `dark`, custom `clusterBkg` is ignored and subgraphs keep the default cream/yellow fill.
 *
 * @param {string} tmpDir Temp dir for `mermaid-config.json`
 * @param {'default'|'dark'} mmdcTheme Palette from {@link mermaidCliTheme} (light vs dark HTML)
 * @returns {Promise<string>} Path to written JSON
 */
async function writeMmdcMermaidConfigFile(tmpDir, mmdcTheme) {
	const cfgPath = path.join(tmpDir, 'mermaid-config.json');
	const isDark = mmdcTheme === 'dark';
	/** Flowchart cluster + node colors; `darkMode` drives derived variables in `theme-base`. */
	...
	const cfg = { theme: 'base', themeVariables };
	await fs.promises.writeFile(cfgPath, JSON.stringify(cfg), 'utf8');
	return cfgPath;
}
...
async function runMmdcToSvg(source, opts) {
	...
	try {
		...
		const mermaidConfigFile = await writeMmdcMermaidConfigFile(tmpDir, palette);
		await execFileAsync(
			process.execPath,
			[
				cliJs,

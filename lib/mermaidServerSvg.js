...
/**
 * Mermaid JSON for `mmdc -c` — tones down default yellow subgraphs (`#ffffde`) to match
 * autodoc HTML light/dark surfaces (see `lib/htmlRenderer.js` `:root` / `body.dark`).
 *
 * @param {string} tmpDir Temp dir for `mermaid-config.json`
 * @param {'default'|'dark'} mmdcTheme From {@link mermaidCliTheme}
...
async function writeMmdcMermaidConfigFile(tmpDir, mmdcTheme) {
	...
	/** Flowchart cluster (subgraph) — align with autodoc theme, not Mermaid’s yellow. */
	...
	await fs.promises.writeFile(cfgPath, JSON.stringify({ themeVariables }), 'utf8');
	return cfgPath;
}
...
async function runMmdcToSvg(source, opts) {
	...
	try {
		...
		const mermaidConfigFile = await writeMmdcMermaidConfigFile(tmpDir, theme);
		await execFileAsync(
			process.execPath,
			[
				cliJs,

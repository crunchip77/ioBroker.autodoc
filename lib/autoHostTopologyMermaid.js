...
function buildAutoHostTopologyMermaid(hostsMap, opts) {
	...
	/*
	 * Single host → TB: instances form a compact vertical column.
	 * Multiple hosts → LR: host boxes appear side-by-side; direction TB inside each
	 * subgraph stacks instances vertically so nodes don't form a single wide row.
	 * Without edges between nodes Mermaid ignores direction inside a subgraph, so for
	 * the single-host case we rely on the outer TB direction instead.
	 */
	const multiHost = hostOrder.length > 1;
	const outerDir = multiHost ? 'LR' : 'TB';
	const lines = [`flowchart ${outerDir}`];
